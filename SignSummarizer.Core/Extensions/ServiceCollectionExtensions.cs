...
public static class ServiceCollectionExtensions
{
        ...
        HandSide dominantHand = HandSide.Right)
    {
        services.AddSingleton<IModelLoader>(sp =>
            new ModelLoader(
                sp.GetRequiredService<ILogger<ModelLoader>>(),
                modelsDirectory));

        services.AddSingleton<ICanonicalizationService>(sp =>
            new CanonicalizationService(
                sp.GetRequiredService<ILogger<CanonicalizationService>>(),
                dominantHand));
        ...
        services.AddSingleton<IHandDetectionService>(sp =>
            new HandDetectionService(
                ...
                sp.GetRequiredService<ILogger<HandDetectionService>>()));
        ...
        string manifestPath)
    {
        var loader = new SignWaveManifestLoader(
            NullLogger<SignWaveManifestLoader>.Instance);
