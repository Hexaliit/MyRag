...
public sealed class SignWaveCoordinator : ISignWaveCoordinator
{
    ...
    {
        if (_waves.TryRemove(name, out var wave)) _logger.LogInformation("Unregistered wave: {Wave}", name);
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (handDetectionService == null) yield break;
    ...
    private void UpdateAtomFromResult(SignAtom atom, SignWaveResult result, SignWaveContext context)
    {
        ...
        if (result.GetData<float[]>("embedding") is var embedding && embedding != null) atom.PoseEmbedding = embedding;
        ...
        if (result.GetScalar<float>("confidence") is var confidence &&
            confidence != 0) // Value type check? confidence is float?
