...
public sealed class VisionLlmService : IVisionLlmService
{
    ...
    public async Task<bool> InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            IsInitialized = _captionRunner != null;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var defaultPrompt = prompt ??
                                "Describe this sign language gesture in detail, including hand position, shape, movement, and any facial expressions.";
    ...
    {
        if (_captionRunner == null) return "Florence-2 model not available. Description unavailable.";
    ...
    private float[] PreprocessImage(string imagePath)
    {
        try
        {
            ...
            Cv2.Resize(image, resized, new Size(384, 384));
            Cv2.CvtColor(resized, normalized, ColorConversionCodes.BGR2RGB);

            var input = new float[3 * 384 * 384];

            var totalBytes = (int)(normalized.Total() * normalized.ElemSize());
            var pixelData = new byte[totalBytes];
            Marshal.Copy(normalized.Data, pixelData, 0, totalBytes);

            for (var i = 0; i < pixelData.Length; i++) input[i] = pixelData[i] / 255.0f;
