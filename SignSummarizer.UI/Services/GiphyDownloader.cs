...
public sealed class GiphyDownloader : IGiphyDownloader
{
    ...
    {
        var url =
            $"{BaseUrl}?api_key={ApiKey}&q={Uri.EscapeDataString(query)}&limit={limit}&offset={offset}&rating={rating}&lang=en&bundle=messaging_non_clips";
        ...
        {
            _logger.LogError("Giphy API error: {StatusCode} - {ReasonPhrase}", response.StatusCode,
        ...
        {
            _logger.LogError("Giphy API returned error: {Status} - {Message}", giphyResponse?.Meta?.Status,
                giphyResponse?.Meta?.Msg);
            return new List<GiphyGif>();
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await File.WriteAllBytesAsync(outputPath, await response.Content.ReadAsByteArrayAsync(cancellationToken),
            cancellationToken);
