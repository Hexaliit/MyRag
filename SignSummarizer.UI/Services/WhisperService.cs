...
public sealed class WhisperService : IWhisperService
{
    ...
    private readonly WhisperModel _model = WhisperModel.TinyEn;
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var segment in chunkingMode switch
                       {
                           WhisperChunkingMode.FullFile => TranscribeFullFileAsync(audioPath, language,
                               cancellationToken),
                           WhisperChunkingMode.ReadAhead => TranscribeReadAheadAsync(audioPath, language, config,
                               cancellationToken),
                           WhisperChunkingMode.MinimalOverlap => TranscribeMinimalOverlapAsync(audioPath, language,
                               config, cancellationToken),
                           WhisperChunkingMode.VADBased => TranscribeVADBasedAsync(audioPath, language, config,
                               cancellationToken),
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        foreach (var segment in speakerSegments) yield return segment;
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var productionTask =
            Task.Run(() => ProduceChunksAsync(audioPath, language, config, readAheadBuffer, cancellationToken),
                cancellationToken);

        await foreach (var segment in readAheadBuffer.Reader.ReadAllAsync(cancellationToken)) yield return segment;
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (chunkStart < audioDuration)
        {
            ...
            foreach (var segment in segments) await output.Writer.WriteAsync(segment, cancellationToken);
    ...
    {
        var analysis = FFProbe.AnalyseAsync(audioPath).GetAwaiter().GetResult();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (isInSpeech) regions.Add((currentStart, duration));
    ...
    private float CalculateAudioEnergy(string audioPath, TimeSpan start, TimeSpan duration)
    {
        try
        {
            ...
            using var reader = new BinaryReader(File.OpenRead(chunkPath));
