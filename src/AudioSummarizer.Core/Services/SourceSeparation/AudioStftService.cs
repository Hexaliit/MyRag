...
public class AudioStftService
{
    ...
    public float[] ComputeIstft(float[,,] spectrogram, int outputLength)
    {
        ...
        for (var frame = 0; frame < numFrames; frame++)
        {
            ...
            for (var bin = 1; bin < numBins - 1; bin++) fullSpectrum[NFFt - bin] = Complex.Conjugate(fullSpectrum[bin]);

            // Inverse FFT (modifies array in-place)
            FFT.Inverse(fullSpectrum);

            // Overlap-add with window
            for (var i = 0; i < NFFt && startIdx + i < outputLength; i++)
            {
        ...
        for (var i = 0; i < outputLength; i++) result[i] = windowSum[i] > 1e-8 ? (float)(output[i] / windowSum[i]) : 0f;
