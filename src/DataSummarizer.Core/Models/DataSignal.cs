...
public static class DataSignalKeys
{
    ...
    public static string SchemaType(string column) => $"schema.{column}.type";
    public static string SchemaNullable(string column) => $"schema.{column}.nullable";

    // Profile pattern for per-column signals
    public static string ProfileCardinality(string column) => $"profile.{column}.cardinality";
    public static string ProfileTopValues(string column) => $"profile.{column}.top_values";
    public static string ProfileDistribution(string column) => $"profile.{column}.distribution";

    // Quality pattern for per-column signals
    public static string QualityNullRate(string column) => $"quality.{column}.null_rate";
    public static string QualityCompleteness(string column) => $"quality.{column}.completeness";

    // Statistics pattern for per-column signals
    public static string StatsMin(string column) => $"stats.{column}.min";
    public static string StatsMax(string column) => $"stats.{column}.max";
    public static string StatsMean(string column) => $"stats.{column}.mean";
    public static string StatsStdDev(string column) => $"stats.{column}.std_dev";
    public static string StatsQuartiles(string column) => $"stats.{column}.quartiles";

    // Outlier pattern for per-column signals
    public static string OutlierCount(string column) => $"outlier.{column}.count";
    public static string OutlierIqrBounds(string column) => $"outlier.{column}.iqr_bounds";

    // Correlation pattern
    public static string Correlation(string col1, string col2) => $"correlation.{col1}_{col2}";

    // PII pattern for per-column signals
    public static string PiiDetected(string column) => $"pii.{column}.detected";
    public static string PiiType(string column) => $"pii.{column}.type";
}
