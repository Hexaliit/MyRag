...
public sealed class SourcePluginRegistry
{
    ...
    public void Register(ISourcePlugin plugin)
    {
        ...
        foreach (var key in plugin.Metadata.Keys)
        {
            ...
            {
                Debug.WriteLine($"[PluginRegistry] Warning: duplicate key '{key}' — skipping (already registered by {_byKey[key.ToLowerInvariant()].Metadata.DisplayName})");
            }
        }
    }
    ...
    public ISourcePlugin? FindByKey(string key)
        => _byKey.GetValueOrDefault(key.ToLowerInvariant());
    ...
    public IReadOnlyList<ISourcePlugin> FindByCapability(SourceCapabilities capability)
        => _all.Where(p => p.Metadata.Capabilities.HasFlag(capability)).ToList();

    /// <summary>
    /// Find plugins that require no API keys.
    /// </summary>
    public IReadOnlyList<ISourcePlugin> FindFreePlugins()
        => _all.Where(p => p.Metadata.RequiredApiKeys.Count == 0).ToList();

    /// <summary>
    /// Check if a key is registered.
    /// </summary>
    public bool HasKey(string key)
        => _byKey.ContainsKey(key.ToLowerInvariant());
