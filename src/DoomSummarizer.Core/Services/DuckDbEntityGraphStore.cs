...
public class DuckDbEntityGraphStore : IEntityGraphStore
{
    ...
    private async Task CreateTablesAsync()
    {
        ...
        try { await ExecAsync($"ALTER TABLE item_embeddings ADD COLUMN entity_profile FLOAT[{_dim}]"); }
        ...
        // HNSW indexes
        try { await ExecAsync("CREATE INDEX IF NOT EXISTS entity_emb_hnsw ON entities USING HNSW (embedding) WITH (metric = 'cosine')"); }
        catch { }

        try { await ExecAsync("CREATE INDEX IF NOT EXISTS item_entity_profile_hnsw ON item_embeddings USING HNSW (entity_profile) WITH (metric = 'cosine')"); }
        catch { }
    }
    ...
    public async Task<List<(GraphEntity entity, float similarity)>> FindSimilarEntitiesAsync(float[] queryEmbedding, int topK = 10)
    {
    ...
    public async Task<List<(string entityId, string name, float confidence, int mentions)>> GetEntitiesForItemAsync(string itemId)
    {
