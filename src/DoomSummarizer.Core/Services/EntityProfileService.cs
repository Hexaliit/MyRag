...
public class EntityProfileService
{
    ...
    public EntityProfileService(IEmbeddingService embedding, IEntityGraphStore vectorStore,
    ...
    /// </returns>
    public async Task<(float[] profile, List<(string entityId, string name, float weight)> topEntities)>
            ...
            Dictionary<string, float[]>? newlyComputedEmbeddings = null)
    {
        ...
        foreach (var (entityId, name, type, confidence, mentions) in docEntities)
        {
            ...
            else
            {
                ...
                if (newlyComputedEmbeddings != null && !wasCached) newlyComputedEmbeddings[entityId] = entityEmbed;
            }
        ...
        foreach (var (_, _, weight, entityEmbed) in entityWeights) VectorMath.AddScaled(profile, entityEmbed, weight);
