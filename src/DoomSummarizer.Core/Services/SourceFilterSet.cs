...
public sealed class SourceFilterSet
{
    ...
    {
        return string.Equals(source, ManualTag, StringComparison.OrdinalIgnoreCase)
    ...
    {
        return name.Contains(':') ? name : $"crawl:{name}";
    }
    ...
    {
        return glob.Replace(@"\", @"\\").Replace("%", @"\%").Replace("_", @"\_")
            .Replace("*", "%").Replace("?", "_");
    }
