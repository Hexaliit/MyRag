...
public partial class StorageService
{
    ...
    public async Task LogQueryAsync(string queryText, float[]? queryEmbedding, string? vibe, List<string> itemIds)
    {
        ...
        cmd.Parameters.AddWithValue("@embedding",
            queryEmbedding != null ? EmbeddingCompat.ToBytes(queryEmbedding) : DBNull.Value);
    ...
    /// </summary>
    public async Task<QueryMatch?> FindSimilarQueryAsync(float[] queryEmbedding, double threshold = 0.85,
        int maxAgeHours = 4)
    {
        var cutoff = DateTimeOffset.UtcNow.AddHours(-maxAgeHours).ToString("O");
        await using var cmd = _connection!.CreateCommand();
        cmd.CommandText =
            "SELECT id, query_text, query_embedding, vibe, item_ids, issued_at FROM query_log WHERE issued_at >= @cutoff AND query_embedding IS NOT NULL ORDER BY issued_at DESC LIMIT 50";
        ...
        while (await reader.ReadAsync())
        {
            ...
            if (similarity >= threshold && similarity > bestSim)
            {
                ...
                best = new QueryMatch
                {
                    ...
                    Vibe = reader.IsDBNull(reader.GetOrdinal("vibe"))
                        ? null
                        : reader.GetString(reader.GetOrdinal("vibe")),
    ...
    /// </summary>
    public async Task<Dictionary<string, (int accessCount, DateTimeOffset lastAccessed)>> GetItemUsageAsync(
        List<string> itemIds)
    {
        ...
        foreach (var batch in itemIds.Chunk(50))
        {
            ...
            cmd.CommandText =
                $"SELECT item_id, access_count, last_accessed FROM item_usage WHERE item_id IN ({string.Join(",", placeholders)})";
