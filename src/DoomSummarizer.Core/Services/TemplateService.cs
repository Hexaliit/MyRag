...
public partial class TemplateService
{
    ...
    {
        return _compiledTemplates.Keys;
    }
    ...
    {
        return _templateSources.GetValueOrDefault(name);
    }
    ...
    {
        if (!_compiledTemplates.TryGetValue(templateName, out var template)) template = _compiledTemplates["default"];
    ...
    private static Dictionary<string, string> GetBuiltInTemplates()
    {
        return new Dictionary<string, string>
        {
            ...
            ["blog-newsletter"] = """
                                  ...
                                  *via {{ pick.Source }}*
                                  ...
                                  ### Quick Hits
                                  {% for hit in QuickHits %}
                                  - **[{{ hit.Title }}]({{ hit.Url }})** — {{ hit.OneLiner }}
                                  {% endfor %}
    ...
    private static ValueTask<FluidValue> TruncateWordsFilter(FluidValue input, FilterArguments args,
        TemplateContext ctx)
    {
    ...
    private static ValueTask<FluidValue> SentimentEmojiFilter(FluidValue input, FilterArguments args,
        TemplateContext ctx)
    {
    ...
    private static ValueTask<FluidValue> SentimentLabelFilter(FluidValue input, FilterArguments args,
        TemplateContext ctx)
    {
        var sentiment = (float)input.ToNumberValue();
