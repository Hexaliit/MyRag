...
public sealed class VibeResolver
{
    ...
    /// </param>
    public void LoadLenses(Assembly? embeddedResourceAssembly = null)
    {
        if (_loaded) return;
        _loaded = true;
        ...
        if (embeddedResourceAssembly != null)
            foreach (var resourceName in embeddedResourceAssembly.GetManifestResourceNames()
                ...
                {
                    Debug.WriteLine($"Failed to load embedded lens {resourceName}: {ex.Message}");
                }
        ...
        foreach (var dir in LensDirectories)
        {
            ...
            foreach (var file in Directory.GetFiles(dir, "*.lens.yaml"))
                ...
                {
                    Debug.WriteLine($"Failed to load lens {file}: {ex.Message}");
                }
        }
    }
    ...
    private sealed class LensYamlManifest
    {
        ...
        [YamlMember(Alias = "display_name")] public string DisplayName { get; set; } = "";

        [YamlMember(Alias = "description")] public string Description { get; } = "";

        [YamlMember(Alias = "enabled")] public bool Enabled { get; } = true;

        [YamlMember(Alias = "personality")] public LensPersonality? Personality { get; set; }

        [YamlMember(Alias = "taxonomy")] public LensTaxonomy? Taxonomy { get; set; }
    ...
    {
        [YamlMember(Alias = "tone")] public string? Tone { get; set; }

        [YamlMember(Alias = "spelling_variant")]
        public string? SpellingVariant { get; set; }

        [YamlMember(Alias = "persona")] public string? Persona { get; set; }

        [YamlMember(Alias = "style_notes")] public List<string>? StyleNotes { get; set; }
    ...
    {
        [YamlMember(Alias = "domain")] public string Domain { get; } = "general";

        [YamlMember(Alias = "style")] public string Style { get; } = "conversational";

        [YamlMember(Alias = "audience")] public string Audience { get; set; } = "general";
    }
}