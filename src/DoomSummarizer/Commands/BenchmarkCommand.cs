...
public sealed class BenchmarkCommand : AsyncCommand<BenchmarkCommand.Settings>
{
    public sealed class Settings : CommandSettings
    {
        ...
        [Description(
            "Comma-separated model names to test (e.g. qwen3:4b,gemma3:4b,phi4-mini). If omitted, auto-detects available small models.")]
        ...
        CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            if (!modelsToTest.Any(m =>
                    m.StartsWith(config.Ollama.Model.Split(':')[0], StringComparison.OrdinalIgnoreCase)))
            {
            ...
            if (modelsToTest.Count == 0)
            {
                AnsiConsole.MarkupLine(
                    "[yellow]No candidate models found.[/] Specify models: [grey]benchmark qwen3:4b,gemma3:4b[/]");
                return 1;
            }
        }
            ...
            foreach (var model in modelsToTest)
            {
                ...
                var summary = new ModelBenchmarkSummary
                {
                    ...
                    IsCurrentSentinel = model.Equals(config.Ollama.SentinelModel, StringComparison.OrdinalIgnoreCase) ||
                                        model.StartsWith(config.Ollama.SentinelModel + ":",
                                            StringComparison.OrdinalIgnoreCase)
                };
                ...
                if (sentinelResults.Count > 0)
                {
                    ...
                    summary.SentValidJson = sentinelResults.Count(r =>
                    {
                        ...
                        try
                        {
                            JsonDocument.Parse(resp[start..(end + 1)]);
                            return true;
                        }
                        catch
                        {
    ...
    private static async Task<int> RunLocalBenchmarkAsync(Settings settings, CancellationToken ct)
    {
        ...
        {
            var names =
 settings.Models.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    ...
    private static void DisplayResults(List<ModelBenchmarkSummary> results, bool showSynth, bool showSent,
        DoomConfig config)
    {
        if (showSynth)
        {
        ...
        if (showSynth)
        {
            ...
            if (synthCandidates.Count > 0)
            {
                ...
                AnsiConsole.MarkupLine(
                    $"  [bold]Synthesis:[/] [green]{Markup.Escape(best.Model)}[/] — {best.SynthAvgTokSec:F1} tok/s, valid markdown+URLs{tag}");
                if (!best.IsCurrentSynthesis)
                    AnsiConsole.MarkupLine(
                        $"    To switch: edit [grey]~/.doomsummarizer/config.json[/] → set [cyan]ollama.model[/] = \"{best.Model.Split(':')[0]}:{best.Model.Split(':').ElementAtOrDefault(1) ?? "latest"}\"");
            }
            else
            {
                AnsiConsole.MarkupLine(
                    "  [yellow]Synthesis: No models produced valid markdown+URLs. Stick with current config.[/]");
            }
        }
        ...
        if (showSent)
        {
            ...
            if (sentCandidates.Count > 0)
            {
                ...
                AnsiConsole.MarkupLine(
                    $"  [bold]Sentinel:[/] [green]{Markup.Escape(best.Model)}[/] — {best.SentAvgTokSec:F1} tok/s, 100% valid JSON{tag}");
                if (!best.IsCurrentSentinel)
                    AnsiConsole.MarkupLine(
                        $"    To switch: edit [grey]~/.doomsummarizer/config.json[/] → set [cyan]ollama.sentinelModel[/] = \"{best.Model.Split(':')[0]}:{best.Model.Split(':').ElementAtOrDefault(1) ?? "latest"}\"");
            }
            else
            {
                AnsiConsole.MarkupLine(
                    "  [yellow]Sentinel: No models achieved 100% valid JSON. Stick with current config.[/]");
            }
        }
