...
public sealed partial class PageCommand : AsyncCommand<PageCommand.Settings>
{
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken ct)
    {
        ...
        using var processor =
            await ItemProcessor.CreateAsync(boot.Embedding, boot.Storage, collectionName: "default", ct: ct);
        ...
        await ProgressHelper.RunAsync(async ctx =>
        {
            ...
            try
            {
                ...
                fetchTask.Description =
                    $"[green]Fetched: {Markup.Escape(title.Length > 60 ? title[..57] + "..." : title)}[/]";
            }
            catch (Exception ex)
            {
                fetchTask.Value = 100;
            ...
            // Structural analysis
            if (item.Content != null) item.ContentStructure = MarkdownContentAnalyzer.Analyze(item.Content);
            ...
            else
            {
                ...
                if (contentSnippet.Length > 3000)
                    contentSnippet = TextRankExtractor.ExtractKeySentences(
                        contentSnippet, text => boot.Embedding.EmbedAsync(text).GetAwaiter().GetResult(), 3000);
    ...
    private static string? ExtractTitle(string html)
    {
        ...
        if (ogMatch.Success)
            return WebUtility.HtmlDecode(ogMatch.Groups[1].Value.Trim());
