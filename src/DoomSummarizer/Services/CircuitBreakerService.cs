...
public class CircuitBreakerService : IAsyncDisposable
{
    ...
    public async Task TripCircuitAsync(string service, CircuitFailureType failureType, string? reason = null)
    {
        ...
        AnsiConsole.MarkupLine($"[yellow]{service}: circuit tripped ({failureType}) — {reason}[/]");
    }
    ...
    public void PrintCircuitStatus()
    {
        ...
        {
            AnsiConsole.MarkupLine("[grey]All circuits closed (normal operation)[/]");
            return;
        }
        ...
        {
            var statusColor = e.Status == CircuitStatus.Open ? "red" : "yellow";
            ...
            table.AddRow(
                ...
                e.FailureCount.ToString(),
                retryStr,
                Markup.Escape(Truncate(e.LastFailureReason ?? "", 40))
            );
        }

        AnsiConsole.MarkupLine("[bold]Circuit Breaker Status:[/]");
