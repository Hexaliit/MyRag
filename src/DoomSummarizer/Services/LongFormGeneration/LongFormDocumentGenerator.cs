...
public class LongFormDocumentGenerator
{
        ...
        bool parallel = true)
    {
        ...
        AnsiConsole.MarkupLine("[grey]Long-form: Phase 1 — Preparing evidence corpus...[/]");
        ...
        AnsiConsole.MarkupLine(
            $"[grey]  Corpus: {corpus.Segments.Count} segments from {corpus.Articles.Count} articles, " +
            $"{corpus.GlobalEntities.Count} entities[/]");

        // ── Phase 2: Document Planning (sentinel LLM — 1 call) ──
        AnsiConsole.MarkupLine("[grey]Long-form: Phase 2 — Planning document structure...[/]");
        ...
        AnsiConsole.MarkupLine("[grey]Long-form: Phase 4 — Generating sections...[/]");
        ...
        AnsiConsole.MarkupLine("[grey]Long-form: Phase 5 — Validating output...[/]");

        var validation = OutputValidator.Validate(plan, corpus, _articleProcessor.Embed);

        AnsiConsole.MarkupLine(
            $"[grey]  Grounding: {validation.OverallGroundingScore:P0}, " +
        ...
        {
            AnsiConsole.MarkupLine("[yellow]  Applying auto-fix for validation issues...[/]");
            OutputValidator.AutoFix(plan, validation, corpus);
        }

        // ── Phase 6: Assembly (deterministic) ──
        AnsiConsole.MarkupLine("[grey]Long-form: Phase 6 — Assembling document...[/]");
