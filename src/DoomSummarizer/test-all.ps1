...
if (-not $SkipBuild)
{
    ...
    else
    {
        ...
        if ($Verbose)
        {
            $testBuildOut | ForEach-Object { Write-Host "    $_" -ForegroundColor DarkGray }
        }
    }
}
else
...
if (-not $SkipSmoke)
{
    ...
    else
    {
        ...
        elseif ($r)
        {
            Write-Fail "help: exit code $( $r.ExitCode )"
        }
        ...
        {
            Write-Pass 'config: exits cleanly'
        }
        elseif ($r)
        {
            Write-Fail "config: exit code $( $r.ExitCode )"
        }
        ...
        if ($r -and $r.ExitCode -eq 0)
        {
            Write-Pass 'sources: lists available sources'
        }
        elseif ($r)
        {
            Write-Fail "sources: exit code $( $r.ExitCode )"
        }
        ...
        if ($r -and $r.ExitCode -eq 0)
        {
            Write-Pass 'show: exits cleanly'
        }
        elseif ($r)
        {
            Write-Fail "show: exit code $( $r.ExitCode )"
        }
        ...
        if ($r)
        {
            if ($r.ExitCode -eq 0)
            {
                Write-Pass 'scroll (quiet, no-llm): completes'
            }
            elseif ($r.Stderr -match 'No items|Error|failed')
            {
                Write-Pass 'scroll (quiet, no-llm): exits with expected warning'
            }
            else
            {
                Write-Fail "scroll (quiet, no-llm): exit $( $r.ExitCode )"
            }
        }
        ...
        if ($r -and $r.ExitCode -eq 0 -and $r.Stdout.Trim().Length -gt 0)
        {
            ...
            if ($cleaned -match '^\s*[\[{]')
            {
                Write-Pass 'scroll (json): produces valid JSON'
            }
            else
            {
                Write-Fail 'scroll (json): output is not JSON'
            }
        }
        elseif ($r -and $r.ExitCode -eq 0)
        {
            Write-Pass 'scroll (json): exits cleanly'
        }
        elseif ($r)
        {
            Write-Fail "scroll (json): exit $( $r.ExitCode )"
        }
        ...
        if ($r)
        {
            if ($r.ExitCode -eq 0 -or $r.Stderr -match 'No items|No local|empty')
            {
                Write-Pass 'scroll (local): exits cleanly'
            }
            else
            {
                Write-Fail "scroll (local): exit $( $r.ExitCode )"
            }
        }

        # Scroll debug
        $r = Run-Cli -CliArgs 'scroll "test" -q --no-llm --limit 2 --debug' -Name 'scroll-debug' -TimeoutSec 60
        if ($r -and $r.ExitCode -ge 0)
        {
            Write-Pass 'scroll (debug): no crash'
        }
        elseif ($r)
        {
            Write-Fail "scroll (debug): exit $( $r.ExitCode )"
        }
        ...
        if ($r)
        {
            Write-Pass "page (invalid URL): handled (exit: $( $r.ExitCode ))"
        }
        ...
        if ($r -and $r.ExitCode -ge 0)
        {
            Write-Pass 'scroll (briefing): flag accepted'
        }
        elseif ($r)
        {
            Write-Fail "scroll (briefing): exit $( $r.ExitCode )"
        }
        ...
        if ($r -and $r.ExitCode -ge 0)
        {
            Write-Pass 'scroll (entities): flag accepted'
        }
        elseif ($r)
        {
            Write-Fail "scroll (entities): exit $( $r.ExitCode )"
        }
    }
}
else
