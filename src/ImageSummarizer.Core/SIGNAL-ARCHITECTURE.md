...
**Key insight**: Captions do NOT benefit from heuristic context. A/B testing showed that simple prompts produce
equivalent or better captions than context-heavy prompts. The LLM sees the image directly - it doesn't need to be told
...
Even in fast mode, we compute basic identity signals for caching. Caching happens in a **background coordinator** to
...
**Conclusion**: The Vision LLM sees the actual image - it doesn't need to be told what it's looking at. Context can
actually confuse the model or cause prompt leakage.
