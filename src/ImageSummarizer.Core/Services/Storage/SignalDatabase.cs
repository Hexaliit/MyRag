...
public class SignalDatabase : ISignalDatabase, IDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            try
            {
                foreach (var signal in profile.GetAllSignals()) await InsertSignalAsync(imageId, signal, ct);
    ...
    public async Task<DynamicImageProfile?> LoadProfileAsync(string sha256, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!await imageReader.ReadAsync(ct)) return null; // Image not found
            ...
            while (await signalsReader.ReadAsync(ct))
            {
                var signal = new Signal
                {
                    ...
                    Value = signalsReader.IsDBNull(2)
                        ? null
                        : JsonSerializer.Deserialize<object>(signalsReader.GetString(2)),
                    Confidence = signalsReader.GetDouble(3),
                    Source = signalsReader.GetString(4),
                    Timestamp = signalsReader.GetDateTime(5),
                    Tags = signalsReader.IsDBNull(6)
                        ? null
                        : JsonSerializer.Deserialize<List<string>>(signalsReader.GetString(6)),
                    Metadata = signalsReader.IsDBNull(7)
                        ? null
                        : JsonSerializer.Deserialize<Dictionary<string, object>>(signalsReader.GetString(7))
                };
        ...
        CancellationToken ct = default)
    {
        ...
        if (imageId == null) throw new InvalidOperationException($"Image with SHA256 {sha256} not found");
    ...
    public async Task StoreDiscriminatorScoreAsync(DiscriminatorScore score, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            cmd.Parameters.AddWithValue("@accepted",
                score.Accepted.HasValue ? score.Accepted.Value ? 1 : 0 : DBNull.Value);
            cmd.Parameters.AddWithValue("@feedback", score.Feedback ?? (object)DBNull.Value);
            cmd.Parameters.AddWithValue("@signalContributionsJson",
                JsonSerializer.Serialize(score.SignalContributions));
