...
public class UnifiedVisionService
{
        ...
        ILoggerFactory loggerFactory)
    {
        ...
        _clients = new Dictionary<string, IVisionClient>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["Anthropic"] =
                new AnthropicVisionClient(configuration, loggerFactory.CreateLogger<AnthropicVisionClient>()),
    ...
    public IVisionClient GetClient(string? provider = null)
    {
        ...
        if (_clients.TryGetValue(providerName, out var client)) return client;

        throw new InvalidOperationException(
            $"Vision provider '{providerName}' not found. Available providers: {string.Join(", ", _clients.Keys)}");
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(modelSpec)) return (null, string.Empty);
        ...
        if (parts.Length == 2)
        {
            ...
            if (_clients.ContainsKey(provider)) return (provider, model);

            _logger.LogWarning("Unknown provider '{Provider}' in model spec '{ModelSpec}', treating as Ollama model",
                provider, modelSpec);
