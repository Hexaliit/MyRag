    ...
    ILogger<SegmentGraphService> logger) : ISegmentGraphService
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            query = query.Where(l => linkTypes.Contains(l.LinkType));
        }
        ...
        CancellationToken ct = default)
    {
        ...
        return new SegmentGraphStats(
            ...
            AverageWeight: links.Count > 0 ? links.Average(l => l.Weight) : 0,
    ...
    private List<SegmentLink> BuildHeadingLinks(Guid documentId, IReadOnlyList<Segment> segments)
    {
        ...
        foreach (var group in byHeading)
        {
            ...
            for (int i = 0; i < segmentsInGroup.Count; i++)
            {
                for (int j = i + 1; j < segmentsInGroup.Count; j++)
                {
                    links.Add(new SegmentLink
                    {
                        Id = Guid.NewGuid(),
    ...
    private List<SegmentLink> BuildSemanticLinks(Guid documentId, IReadOnlyList<Segment> segments)
    {
        ...
        for (int i = 0; i < segmentsWithEmbeddings.Count; i++)
        {
            for (int j = i + 1; j < segmentsWithEmbeddings.Count; j++)
            {
...
public interface ISegmentGraphService
{
    ...
    Task<List<ConnectedSegment>> GetConnectedSegmentsAsync(string segmentHash, int maxResults = 10, string[]? linkTypes = null, CancellationToken ct = default);
    Task<List<string>> ExpandRetrievalAsync(IEnumerable<string> segmentHashes, int expansionDepth = 1, double minWeight = 0.5, int maxExpansions = 5, CancellationToken ct = default);
