...
public class SegmentSelector
{
    ...
    public List<Segment> SelectForEvidence(IReadOnlyList<Segment> segments)
    {
        ...
        Console.WriteLine(
            $"[SegmentSelector] Step 1: After salience filter: {salientSegments.Count} (min={minSegments}, maxSalience={maxSalience:F4})");
        ...
        {
            Console.WriteLine(
                $"[SegmentSelector] Step 2.5: Deduplication too aggressive, adding evenly-sampled segments to meet min={minSegments}");
            ...
            for (var i = 0; i < orderedByPosition.Count && dedupedSegments.Count < minSegments; i += step)
    ...
    private List<Segment> SelectForCoverage(List<Segment> segments)
    {
        ...
        if (result.Count < minCoverage)
        {
            ...
            for (var i = 0; i < orderedSegments.Count && result.Count < minCoverage; i += step)
                result.Add(orderedSegments[i]);
        }

        // Cap at maximum
        if (result.Count > _maxSegmentsPerDocument) result = result.Take(_maxSegmentsPerDocument).ToList();
    ...
    private static List<string> ExtractEntities(Segment segment)
    {
        ...
        if (!string.IsNullOrWhiteSpace(segment.SectionTitle)) features.Add($"section:{segment.SectionTitle}");
        ...
        if (segment.PageNumber.HasValue) features.Add($"page:{segment.PageNumber}");
