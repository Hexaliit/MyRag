...
public record SchemaContext
{
    ...
    public string ToPromptDescription()
    {
        ...
        if (EvidenceTypes.Count > 0)
            parts.Add(
                $"Evidence available: {string.Join(", ", EvidenceTypes.Take(10))}{(EvidenceTypes.Count > 10 ? "..." : "")}");

        if (EntityTypes.Count > 0)
            parts.Add(
                $"Entity types: {string.Join(", ", EntityTypes.Take(15))}{(EntityTypes.Count > 15 ? "..." : "")}");

        if (RelationshipTypes.Count > 0)
            parts.Add(
                $"Relationship types: {string.Join(", ", RelationshipTypes.Take(10))}{(RelationshipTypes.Count > 10 ? "..." : "")}");
    ...
    {
        return EvidenceTypes.Contains(type) ||
