...
public class BrowserUploadTests : IAsyncLifetime
{
    ...
    public async Task ChatUI_ShouldSendMessage_WithoutErrors()
    {
        ...
        _page.Error += (_, e) => { pageErrors.Add($"[PageError] {e}"); };
        ...
        Console.WriteLine("\n=== Console Messages ===");
        foreach (var msg in consoleMessages) Console.WriteLine(msg);

        Console.WriteLine("\n=== Page Errors ===");
        foreach (var err in pageErrors) Console.WriteLine(err);
        ...
        if (jsErrors.Any())
        {
            ...
            foreach (var err in jsErrors) Console.WriteLine(err);
        }
    ...
    public async Task Upload_LargeBlogPost_ShouldSucceed()
    {
        ...
        // Assert
        if (result.TryGetProperty("error", out var err)) throw new Exception($"Upload failed: {err.GetString()}");
