    ...
    ILogger<CommunityController> logger) : ControllerBase
{
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            return TypedResults.Ok(new CommunityDetectionResponse(
                result.CommunitiesDetected,
                result.EntitiesAssigned,
                result.Modularity,
                result.ProcessingTime.TotalSeconds,
                new ApiMeta(DateTimeOffset.UtcNow, sw.ElapsedMilliseconds)));
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> Get(Guid id, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrEmpty(community.Features))
            try
            {
                features = JsonSerializer.Deserialize<object>(community.Features);
            }
            catch
            {
                /* ignore parse errors */
