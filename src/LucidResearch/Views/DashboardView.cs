...
public static class DashboardView
{
    public static Visual Create(AppState appState)
    {
        return new VStack(
            ...
            new HStack(
                ...
                new Group("Metrics")
                    .Content(new VStack(
                        new TextBlock(() => $"  Status:      {FormatStatus(appState.Status.Value)}"),
                        new TextBlock(() => $"  Topic:       {Truncate(appState.Topic.Value, 40)}"),
                        new TextBlock(() => $"  Iteration:   {appState.Iteration.Value}"),
                        new TextBlock(() => $"  Fetched:     {appState.PapersFetched.Value}"),
                        new TextBlock(() => $"  Ingested:    {appState.PapersIngested.Value}"),
                        new TextBlock(() => $"  Failed:      {appState.PapersFailed.Value}"),
                        new TextBlock(() => $"  Frontier:    {appState.FrontierSize.Value}"),
                        new TextBlock(() => $"  Seen:        {appState.SeenCount.Value}"),
                        new TextBlock(() => $"  Entities:    {appState.TotalEntities.Value}"),
                        new TextBlock(() => $"  Orphans:     {appState.OrphanCitations.Value}")
                    ).Spacing(0)),
                ...
                new Group("Convergence")
                    .Content(new VStack(
                        new TextBlock(() =>
                            $"  NewInfoRatio: {appState.NewInfoRatio.Value:F3}"),
    ...
    private static string FormatSparkline(List<double> values)
    {
        ...
        // Single-pass min/max instead of two LINQ enumerations
        ...
        // stackalloc avoids heap allocation for sparkline chars
    ...
    private static string FormatActivityLog(List<string> log)
    {
        ...
        // StringBuilder instead of Skip + Select + Join
