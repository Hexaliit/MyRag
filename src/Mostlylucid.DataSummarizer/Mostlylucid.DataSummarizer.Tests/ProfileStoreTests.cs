...
public class ProfileStoreTests : IDisposable
{
    ...
    public void GetHistory_ShouldReturnProfilesInChronologicalOrder()
    {
        ...
        // Act
        var history = _store.GetHistory(info1.SchemaHash, 10);
    ...
    public void FindSimilar_ShouldFindProfilesAboveThreshold()
    {
        ...
        // Act
        var similar = _store.FindSimilar(profile1, 0.7);
    ...
    public void FindMostSimilarByCentroid_ShouldReturnClosestProfile()
    {
        ...
        var profile1 = CreateTestProfileWithStats("a.csv", 1000, 100, 20);
        var profile2 = CreateTestProfileWithStats("b.csv", 1000, 105, 22); // Very close to profile1
        var profile3 = CreateTestProfileWithStats("c.csv", 1000, 500, 100); // Far from profile1

        _store.Store(profile1);
        _store.Store(profile2);
        _store.Store(profile3);

        var query = CreateTestProfileWithStats("query.csv", 1000, 102, 21);
    ...
    public void StoreSegment_ShouldStoreWithSegmentMetadata()
    {
        ...
        // Act
        var info = _store.StoreSegment(profile, "Q1-2024", "quarter = 1 AND year = 2024");
    ...
    {
        // Arrange
        var profile1 = CreateTestProfileWithStats("db://table1", 1000, 100, 20);
        var profile2 = CreateTestProfileWithStats("db://table2", 1000, 200, 40); // Different stats
    ...
    {
        // Arrange
        var profile = CreateTestProfileWithStats("test.csv", 1000, 50, 10);
    ...
    {
        // Arrange
        var baseProfile = CreateTestProfileWithStats("base.csv", 1000, 100, 20);
        var closeProfile = CreateTestProfileWithStats("close.csv", 1000, 105, 22);
        var farProfile = CreateTestProfileWithStats("far.csv", 1000, 500, 100);
        ...
        // Act
        var nearby = _store.FindWithinDistance(baseProfile);
