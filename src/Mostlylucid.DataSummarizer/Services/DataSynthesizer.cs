...
public static class DataSynthesizer
{
        ...
        for (var i = 0; i < rows; i++)
        {
            ...
            foreach (var col in profile.Columns)
            {
                ...
                if (correlatedColumns != null && correlatedValues != null)
                {
                    ...
                    if (colIdx >= 0) correlatedValue = correlatedValues[i][colIdx];
                }
    ...
    private static string GenerateId(ColumnProfile col, int rowIndex)
    {
        ...
        if (col.TextPatterns.Any(p => p.PatternType == TextPatternType.Uuid)) return Guid.NewGuid().ToString();
        ...
        double? correlatedValue = null)
    {
        ...
        if (col.ZeroCount > 0 && col.Count > 0)
        {
            ...
            if (rnd.NextDouble() < zeroRate) return "0";
        }
    ...
    private static double GenerateFromHistogram(NumericHistogram histogram, Random rnd)
    {
        ...
        for (var i = 0; i < histogram.BinCounts.Count; i++)
        {
    ...
    private static string GenerateDateWithGranularity(TimeGranularity granularity, DateTime min, DateTime max,
        Random rnd)
    {
        var span = max - min;
        var t = rnd.NextDouble();
        ...
        return granularity switch
        {
            ...
            TimeGranularity.Hourly => new DateTime(val.Year, val.Month, val.Day, val.Hour, 0, 0).ToString(
                "yyyy-MM-ddTHH:00:00"),
