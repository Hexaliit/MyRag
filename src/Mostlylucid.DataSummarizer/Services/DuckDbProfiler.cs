...
public class DuckDbProfiler : IDisposable
{
    ...
    public async Task<DataProfile> ProfileAsync(DataSource source)
    {
        ...
        if (_options.IgnoreErrors) source.IgnoreErrors = true;
                                    ...
                                    ex.Message.Contains("CSV Error")))
        {
            ...
            AnsiConsole.MarkupLine(
                "[yellow]Warning:[/] CSV contains malformed data. Retrying with error tolerance (some rows may be skipped).");
            if (_verbose)
            {
            ...
            profile.Alerts.Insert(0, new DataAlert
            {
                ...
                Message =
                    "CSV contained malformed rows that were skipped during analysis. Use --ignore-errors to suppress this warning."
            });
    ...
    private async Task<DataProfile> ProfileInternalAsync(DataSource source, string readExpr, Stopwatch stopwatch)
    {
        ...
        if (_verbose && profile.Columns.Count < allColumns.Count)
            Console.WriteLine(
                $"[Profile] Analyzing {profile.Columns.Count} of {allColumns.Count} columns (use --columns to specify)");
        ...
        if (!_options.FastMode || _options.Depth == ProfileDepth.Background)
        {
            piiTask = Task.Run(async () =>
            {
                ...
                if (_onnxConfig?.Enabled == true)
                    ...
                    {
                        if (_verbose)
                            Console.WriteLine($"[DuckDbProfiler] Failed to enable ONNX classifier: {ex.Message}");
                    }
        ...
        if (correlationsTask != null) profile.Correlations = correlationsTask.Result;
        ...
        if (conditionalTask != null)
        {
            ...
            if (_verbose && profile.ConditionalTables.Count > 0)
                Console.WriteLine(
                    $"[Profile] Found {profile.ConditionalTables.Count} categorical relationships for synthesis");
        }
        ...
        if (!_options.FastMode || _options.Depth == ProfileDepth.Background)
        {
            ...
            foreach (var col in profile.Columns) await patternDetector.EnrichWithPatternsAsync(col, profile);
        ...
        if (!string.IsNullOrWhiteSpace(_options.TargetColumn))
        {
            ...
            if (targetProfile != null)
            {
                ...
                var targetColumn = profile.Columns.FirstOrDefault(c =>
                    c.Name.Equals(_options.TargetColumn, StringComparison.OrdinalIgnoreCase));
                if (targetColumn != null) targetColumn.SemanticRole = SemanticRole.Target;
    ...
    private List<ColumnProfile> FilterAndPrioritizeColumns(List<ColumnProfile> allColumns)
    {
        ...
        if (!string.IsNullOrWhiteSpace(_options.TargetColumn))
            ...
            {
                var targetCol = allColumns.FirstOrDefault(c =>
                    c.Name.Equals(_options.TargetColumn, StringComparison.OrdinalIgnoreCase));
                if (targetCol != null) result.Insert(0, targetCol);
            }

        // Calculate interest scores and limit if too many columns
        if (_options.MaxColumns > 0 && result.Count > _options.MaxColumns)
        {
            foreach (var col in result) col.InterestScore = CalculateInterestScore(col);

            // Keep top N most interesting + any date columns
            var dateColumns = result.Where(c => c.DuckDbType.Contains("DATE") || c.DuckDbType.Contains("TIME"))
                .ToList();
            ...
            if (_verbose)
                Console.WriteLine(
                    $"[Profile] Selected columns by interest: {string.Join(", ", result.Take(10).Select(c => c.Name))}...");
        }
    ...
    /// </summary>
    private async Task<NumericHistogram?> BuildHistogramAsync(string readExpr, string colName, double? min, double? max,
        int numBins)
    {
        ...
        // Generate bin edges
        for (var i = 0; i <= numBins; i++) histogram.BinEdges.Add(min.Value + i * binWidth);
    ...
    private static GenerationPolicy DetermineGenerationPolicy(ColumnProfile col)
    {
        ...
        var piiNamePatterns = new[]
            { "email", "phone", "ssn", "social", "credit", "card", "password", "secret", "token", "address", "ip" };
    ...
    {
        var sql =
            $@"SELECT ""{column}"" FROM {readExpr} WHERE ""{column}"" IS NOT NULL ORDER BY RANDOM() LIMIT {maxRows}";
    ...
    private async Task<(double?, int?)> GetTextStatsAsync(string readExpr, string column)
    {
        ...
        if (await reader.ReadAsync())
        {
            var avg = reader.IsDBNull(0) ? null : TryGetDouble(reader, 0);
    ...
    private async Task<(double? AvgLength, int? MaxLength, int? MinLength, int EmptyCount)> GetTextStatsExtendedAsync(
        string readExpr, string column)
    {
        var sql = $@"
            SELECT 
                AVG(LENGTH(""{column}"")),
        ...
        try
        {
            ...
            if (await reader.ReadAsync())
            {
                ...
                var empty = reader.IsDBNull(3) ? 0 : (int)reader.GetInt64(3);
    ...
    private async Task<List<ColumnCorrelation>> GetCorrelationsAsync(string readExpr, List<ColumnProfile> columns)
    {
        ...
        for (var i = 0; i < numericCols.Count && pairsComputed < maxPairs; i++)
        for (var j = i + 1; j < numericCols.Count && pairsComputed < maxPairs; j++)
        {
        ...
        List<ColumnProfile> columns)
    {
        ...
        if (_verbose)
            Console.WriteLine(
                $"[Profile] Computing conditional tables for {categoricalCols.Count} categorical columns");

        // Evaluate all pairs and find strong relationships
        for (var i = 0; i < categoricalCols.Count && pairsComputed < maxPairs; i++)
        for (var j = 0; j < categoricalCols.Count && pairsComputed < maxPairs; j++)
        {
            if (i == j) continue;
            ...
            {
                if (_verbose)
                    Console.WriteLine($"[Profile] Failed to compute conditional for {parent}->{child}: {ex.Message}");
            }
        }
    ...
    private List<DataAlert> DetectAlerts(DataProfile profile)
    {
        ...
        foreach (var col in profile.Columns)
        {
            ...
            if (col.OutlierCount > 0 && col.Q25.HasValue && col.Q75.HasValue)
            {
                ...
                if (outlierPct > 1)
                {
                    ...
                    alerts.Add(new DataAlert
                    {
                        ...
                        Message =
                            $"{col.OutlierCount:N0} outliers ({outlierPct:F1}%) outside IQR bounds [{lowerBound:F1}, {upperBound:F1}]"
                    });
                }
            }
            ...
            var isProbablyFreeText =
                col.InferredType == ColumnType.Text && col.AvgLength.HasValue && col.AvgLength > 50;
            if (col.InferredType != ColumnType.Id &&
                col.InferredType != ColumnType.DateTime &&
                !isProbablyFreeText &&
                col.UniquePercent > 90 && col.UniqueCount > 100)
            {
                ...
                alerts.Add(new DataAlert
                {
                    ...
                    Message =
                        $"⚠ Potential data leakage: {col.UniquePercent:F1}% unique ({col.UniqueCount:N0} values). " +
                        $"{explanation} " +
            ...
            if (col.InferredType == ColumnType.Categorical && col.TopValues != null && col.TopValues.Count <= 10)
            {
                ...
                if (allNumeric && col.TopValues.Count >= 3)
                    alerts.Add(new DataAlert
                    {
                        ...
                        Message =
                            $"ℹ Ordinal detected: {col.UniqueCount} integer levels - consider treating as ordered numeric"
                    });
            }
            ...
            if (col.InferredType == ColumnType.Numeric && col.TopValues != null)
            {
                ...
                if (zeroValue != null && zeroValue.Percent > 30)
                    alerts.Add(new DataAlert
                    {
                        ...
                        Message =
                            $"ℹ Zero-inflated: {zeroValue.Percent:F1}% zeros - consider two-part model or segmentation"
                    });
            }
        }
        ...
        if (profile.Target != null)
        {
            ...
            if (minorityPct < 20)
            {
                ...
                alerts.Add(new DataAlert
                {
                    ...
                    Message =
                        $"⚠ Target imbalance: {minorityClass.Key}={minorityPct:F1}% vs {majorityClass.Key}={majorityPct:F1}%"
                });
            }
        }
    ...
    {
        var targetColumnProfile =
            profile.Columns.FirstOrDefault(c => c.Name.Equals(targetColumn, StringComparison.OrdinalIgnoreCase));
        ...
        {
            var effect = col.InferredType switch
            {
                ColumnType.Numeric => await AnalyzeNumericEffectAsync(readExpr, col, targetColumn, encoding, totalRows),
                ColumnType.Categorical or ColumnType.Text or ColumnType.Boolean => await AnalyzeCategoricalEffectAsync(
                    readExpr, col, targetColumn, encoding, totalRows, baseRate),
                ColumnType.DateTime => null, // Skip DateTime columns for target analysis (can't compute STDDEV on dates)
                _ => null
            };

            if (effect != null) targetProfile.FeatureEffects.Add(effect);
        }
    ...
    private void AddTargetInsights(DataProfile profile, TargetProfile targetProfile)
    {
        ...
        if (targetProfile.FeatureEffects.Count > 0)
        {
            ...
            profile.Insights.Add(new DataInsight
            {
                ...
                RelatedColumns = new List<string> { targetProfile.ColumnName }.Concat(topEffects.Select(e => e.Feature))
                    .ToList(),
    ...
    private TargetEncoding BuildTargetEncoding(string targetColumn, List<(string Value, long Count)> classes)
    {
        ...
        var negative =
            ordered.FirstOrDefault(c => !c.Value.Equals(positive, StringComparison.OrdinalIgnoreCase)).Value ??
            ordered[0].Value;
    ...
    private async Task<FeatureEffect?> AnalyzeNumericEffectAsync(string readExpr, ColumnProfile column,
        string targetColumn, TargetEncoding encoding, long totalRows)
    {
        await using var cmd = Connection.CreateCommand();
        cmd.CommandText = $"""
        ...
        var pooledStd = Math.Sqrt(
            ((positive.count - 1) * Math.Pow(positive.std, 2) + (negative.count - 1) * Math.Pow(negative.std, 2)) /
        ...
        return new FeatureEffect
        {
            ...
            Support = support,
            Summary =
                $"Average {column.Name} is {positive.mean:F1} for {encoding.PositiveLabel} vs {negative.mean:F1} for {encoding.NegativeLabel} (Δ {meanDiff:F1})",
    ...
    private async Task<FeatureEffect?> AnalyzeCategoricalEffectAsync(string readExpr, ColumnProfile column,
        string targetColumn, TargetEncoding encoding, long totalRows, double baseRate)
    {
        await using var cmd = Connection.CreateCommand();
        cmd.CommandText = $"""
        ...
        return new FeatureEffect
        {
            ...
            Summary =
                $"{column.Name} = {bestCategory} has {encoding.PositiveLabel} rate {bestRate:P1} vs baseline {baseRate:P1} (Δ {bestDelta:P1})",
            Details = new Dictionary<string, double>
            {
    ...
    private List<DataInsight> GenerateStatisticalInsights(DataProfile profile)
    {
        ...
        ApplyInsightScore(overview, 0.2, 1, 0.2);
        ...
        if (strongCorrs.Any())
        {
            ...
            ApplyInsightScore(corrInsight, avgMagnitude,
                strongCorrs.Count / (double)Math.Max(1, profile.Columns.Count));
        ...
        if (criticalAlerts > 0 || warningAlerts > 0)
        {
            var dqInsight = new DataInsight
            {
                ...
                Description =
                    $"Found {criticalAlerts} critical and {warningAlerts} warning-level data quality issues. " +
                    "Review alerts for details.",
                Source = InsightSource.Statistical
            };
            var magnitude = Math.Min(1,
                (criticalAlerts * 1.0 + warningAlerts * 0.5) / Math.Max(1, profile.Columns.Count));
            ApplyInsightScore(dqInsight, magnitude, 1, 0.6);
    ...
    private static void ApplyInsightScore(DataInsight insight, double magnitude, double support = 1,
        double novelty = 0.5)
    {
    ...
    private static void AddModelingHints(DataProfile profile, List<DataInsight> insights)
    {
        ...
        {
            var numericFeatures = profile.Columns.Count(c =>
                c.InferredType == ColumnType.Numeric && c.SemanticRole != SemanticRole.Identifier);
            var catFeatures = profile.Columns.Count(c =>
                c.InferredType == ColumnType.Categorical && c.SemanticRole != SemanticRole.Target);
            ...
            else if (minorityPct < 20) hints.Add("⚠ Moderate imbalance - use stratified splits and F1 score");
        }
        ...
        if (highCardCats.Any())
            hints.Add(
                $"ℹ High-cardinality categoricals ({string.Join(", ", highCardCats.Select(c => c.Name))}) - consider target encoding or embeddings");

        // ID columns that might leak
        var idCols = profile.Columns
            .Where(c => c.InferredType == ColumnType.Id || c.SemanticRole == SemanticRole.Identifier).ToList();
        if (idCols.Any())
        ...
        if (zeroInflated.Any())
            hints.Add(
                $"ℹ Zero-inflated columns ({string.Join(", ", zeroInflated)}) - consider log transform or two-part model");
    ...
    private static void AddPatternInsights(DataProfile profile, List<DataInsight> insights)
    {
        ...
        if (dateCol?.TimeSeries != null)
        {
            ...
            var desc =
                $"Data is a {ts.Granularity.ToString().ToLower()} time series spanning {dateCol.MinDate:yyyy-MM-dd} to {dateCol.MaxDate:yyyy-MM-dd}.";
            ...
            ApplyInsightScore(tsInsight, ts.HasSeasonality ? 0.6 : 0.3);
        ...
        foreach (var col in profile.Columns.Where(c => c.TextPatterns.Count > 0))
        {
            ...
            ApplyInsightScore(textInsight, topPattern.MatchPercent / 100.0,
                col.Count / (double)Math.Max(1, profile.RowCount), 0.4);
            insights.Add(textInsight);
        }

        // Distribution insights
        foreach (var col in profile.Columns.Where(c =>
                     c.Distribution.HasValue && c.Distribution != DistributionType.Unknown))
        {
            ...
            ApplyInsightScore(distInsight, 0.4, col.Count / (double)Math.Max(1, profile.RowCount));
    ...
    private DuckDBConnection Connection => _db?.Connection
    ...
    private static double? TryGetDouble(DbDataReader reader, int ordinal)
    {
        try
        {
