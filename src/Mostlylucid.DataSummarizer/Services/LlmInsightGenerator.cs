...
public class LlmInsightGenerator : IDisposable
{
        ...
        string clarifierSentinelModel = "qwen2.5:1.5b")
    {
        ...
        catch
        {
            ...
            _ollama = new OllamaApiClient(
                new Uri("http://localhost:11434")); // placeholder; guarded by _ollamaAvailable
        }
    }
        ...
        int maxInsights = 5)
    {
        ...
        if (_verbose) Console.WriteLine($"[LLM] Generated {questions.Count} analytical questions");
    ...
    /// </summary>
    public async Task<DataInsight?> AskAsync(string filePath, DataProfile profile, string question,
        string conversationContext = "", bool skipPrecomputedStats = false)
    {
        ...
        if (selection == null)
        {
            ...
            if (autoOpt != null) selection = autoOpt;
        }
        ...
        if (!skipPrecomputedStats)
        {
            ...
            if (statsAnswer != null) return statsAnswer;
        }
    ...
    private DataInsight? TryAnswerFromStats(DataProfile profile, string question)
    {
        ...
        if ((q.Contains("average") || q.Contains("mean")) && !q.Contains("most average") && !q.Contains("closest"))
        {
            ...
            if (!mentionsColumn && !hasEntity && !q.Contains("overall"))
            {
                ...
                if (options.Count > 0)
                {
                    ...
                    foreach (var opt in options) sbClarify.AppendLine($"- {opt}");
                    return new DataInsight
                    {
        ...
        if (HasFilterIndicators(q, profile)) return null; // Need SQL to filter first
        ...
        if (q.Contains("most average") || q.Contains("closest to average") || q.Contains("closest to mean") ||
            ...
            q.Contains("mose average")) return null;
        ...
        if ((q.Contains("average") || q.Contains("mean")) && !q.Contains("most average") && !q.Contains("closest"))
        {
            ...
            if (matchedCol != null)
                return new DataInsight
                {
                    ...
                    Description =
                        $"The overall average {matchedCol.Name} across all {profile.RowCount:N0} rows is **{matchedCol.Mean:F2}** (median: {matchedCol.Median:F2}, std dev: {matchedCol.StdDev:F2})",
                    Source = InsightSource.Statistical,
            ...
            if (((q.Contains("what") || q.Contains("show")) && (q.Contains("average") || q.Contains("mean")) &&
                 q.Contains("overall")) || !HasAnyEntityMention(q, profile))
            {
                ...
                if (numericCols.Count > 0)
                {
                    ...
                    foreach (var col in numericCols)
                        sb.AppendLine(
                            $"- **{col.Name}**: mean={col.Mean:F2}, median={col.Median:F2}, range=[{col.Min:F2}, {col.Max:F2}]");
                    return new DataInsight
                    {
            ...
            && (q.Contains("overall") || !HasFilterIndicators(q, profile)))
        {
            ...
            if (distCols.Count > 0)
            {
                ...
                foreach (var col in distCols)
                {
                    ...
                    sb.AppendLine(
                        $"- **{col.Name}**: {col.Distribution} ({skewDesc}, skewness={col.Skewness:F2}, kurtosis={col.Kurtosis:F2})");
                }
        ...
        if (q.Contains("outlier") && !HasFilterIndicators(q, profile))
        {
            ...
            if (outlierCols.Count > 0)
            {
                ...
                foreach (var col in outlierCols)
                {
                    ...
                    sb.AppendLine(
                        $"- **{col.Name}**: {col.OutlierCount} outliers ({pct:F1}%) outside [{(col.Q25 ?? 0) - 1.5 * iqr:F2}, {(col.Q75 ?? 0) + 1.5 * iqr:F2}]");
                }
    ...
    private static List<string> BuildAverageClarificationOptions(DataProfile profile)
    {
        ...
        var preferred = profile.Columns
            .Where(c => c.InferredType == ColumnType.Categorical && c.UniqueCount > 1 && c.UniqueCount <= 5000 &&
                        c.NullPercent < 90)
        ...
        {
            return c.Name.Contains(token, StringComparison.OrdinalIgnoreCase);
        }
    ...
    private async Task<DataInsight> RefineClarifierWithSentinelAsync(DataInsight clarifier, string question,
        DataProfile profile)
    {
        ...
        var modelToUse = !string.IsNullOrEmpty(_model) && (_model.Contains("1b", StringComparison.OrdinalIgnoreCase) ||
                                                           _model.Contains("mini", StringComparison.OrdinalIgnoreCase))
            ? _model
        ...
        try
        {
            var prompt = $"""
                          ...
                          """;
            var resp = await _ollama.GenerateAsync(new GenerateRequest
            {
                Model = modelToUse,
                Prompt = prompt
            }).StreamToEndAsync();
            ...
            var sb = new StringBuilder();
            sb.AppendLine("Average over which dimension? Pick one:");
            foreach (var opt in refined) sb.AppendLine($"- {opt}");
    ...
    private static bool IsBroadSummaryQuestion(string question)
    {
        ...
        if (IsFollowUpQuestion(q)) return false;
    ...
    private async Task<DataInsight> GenerateProfileSummaryAsync(DataProfile profile, string question,
    ...
    private string BuildProfileSummaryPrompt(DataProfile profile, string question, string conversationContext)
    {
        var sb = new StringBuilder();
        sb.AppendLine(
            "You are a precise data analyst. Give a short, factual summary of the dataset using ONLY the provided profile. Do not speculate or invent columns. Keep it to 3-5 sentences.");
        sb.AppendLine();
    ...
    public async Task<ReportNarrative> GenerateReportNarrativeAsync(DataProfile profile,
        IReadOnlyCollection<string>? focusQuestions)
    {
    ...
    private string BuildNarrativePrompt(DataProfile profile, IReadOnlyCollection<string> focusQuestions)
    {
        var sb = new StringBuilder();
        sb.AppendLine(
            "You are a senior data analyst. Using only the profile below, write a concise summary (3 sentences) and answer each focus question directly.");
        ...
        if (profile.Target != null)
        {
            ...
            foreach (var kvp in profile.Target.ClassDistribution) sb.AppendLine($"- {kvp.Key}: {kvp.Value:P1}");
        }
        ...
        if (focusQuestions.Count > 0)
        {
            ...
            foreach (var question in focusQuestions) sb.AppendLine($"- {question}");
        }
    ...
    private ReportNarrative ParseNarrative(string rawResponse, IReadOnlyCollection<string> focusQuestions)
    {
        ...
        if (json != null)
            try
            {
                ...
                if (doc.RootElement.TryGetProperty("focus", out var focusArray) &&
                    focusArray.ValueKind == JsonValueKind.Array)
    ...
    private string BuildQuestionGenerationPrompt(DataProfile profile)
    {
        ...
        sb.AppendLine(
            "You are a data analyst. Based on the dataset profile below, generate 5-7 insightful analytical questions that would reveal interesting patterns or business insights.");
        sb.AppendLine();
        ...
        foreach (var col in profile.Columns)
        {
            ...
            if (col.InferredType == ColumnType.Numeric && col.Mean.HasValue)
            {
                ...
                sb.Append(
                    $" [range: {col.Min:F0}-{col.Max:F0}, avg: {col.Mean:F1}, std: {col.StdDev:F1}{madText}{skewText}]");
            }
            ...
            if (col.NullPercent > 5) sb.Append($" ({col.NullPercent:F0}% null)");
        ...
        if (profile.Alerts.Count > 0)
        {
            ...
            foreach (var alert in profile.Alerts.Take(5)) sb.AppendLine($"  - {alert.Column}: {alert.Message}");
        }
        ...
        if (profile.Alerts.Count > 0)
        {
            ...
            foreach (var alert in profile.Alerts.Take(3)) sb.AppendLine($"    * {alert.Column}: {alert.Message}");
        }
    ...
    private static void AppendPatternContext(StringBuilder sb, DataProfile profile)
    {
        ...
        if (distributedCols.Any())
        {
            ...
            foreach (var col in distributedCols) sb.AppendLine($"  - DISTRIBUTION: '{col.Name}' is {col.Distribution}");
        }
        ...
        if (textPatternCols.Any())
        {
            ...
            foreach (var col in textPatternCols)
            {
                ...
                sb.AppendLine(
                    $"  - TEXT FORMAT: '{col.Name}' contains {pattern.PatternType} values ({pattern.MatchPercent:F0}%)");
            }
        }
    ...
    private static void AppendAdvancedAnalyticsContext(StringBuilder sb, DataProfile profile)
    {
        ...
        if (strongCorrelations.Count > 0)
        {
            ...
            foreach (var corr in strongCorrelations)
            {
                var direction = corr.Correlation > 0 ? "positive" : "negative";
                sb.AppendLine(
                    $"    - \"{corr.Column1}\" ↔ \"{corr.Column2}\": {corr.Correlation:F2} ({corr.Strength} {direction})");
            }
        }
        ...
        if (trendedCols.Count > 0)
        {
            ...
            foreach (var col in trendedCols)
            {
                ...
                sb.AppendLine(
                    $"    - \"{col.Name}\": {t.Direction} trend (slope={slopeDir}{t.Slope:F4}, R²={t.RSquared:F2})");
            }
        }
        ...
        if (distributedCols.Count > 0)
        {
            ...
            foreach (var col in distributedCols) sb.AppendLine($"    - \"{col.Name}\": {col.Distribution}");
        }
        ...
        if (piiAlerts.Count > 0)
        {
            ...
            foreach (var alert in piiAlerts) sb.AppendLine($"    - \"{alert.Column}\": {alert.Message}");
        }
        ...
        if (cachedResult.DerivedStats != null)
        {
            ...
            if (_verbose)
            {
                ...
                if (cachedResult.DerivedStats.DetectedPatterns?.Count > 0)
                    Console.WriteLine(
                        $"[Enrichment] Patterns: {string.Join("; ", cachedResult.DerivedStats.DetectedPatterns)}");
            }
        }
    ...
    private static string? CheckSqlGuardrails(string sql)
    {
        ...
        foreach (var forbidden in ForbiddenSqlPatterns)
        {
            ...
            if (Regex.IsMatch(upper, pattern)) return $"Forbidden operation: {forbidden}";
        }

        // Must start with SELECT (read-only)
        if (!upper.TrimStart().StartsWith("SELECT")) return "Only SELECT queries are allowed";
        ...
        string? errorHint)
    {
        ...
        foreach (var col in profile.Columns.Where(c => c.InferredType == ColumnType.Numeric && c.Mean.HasValue))
            sb.AppendLine(
                $"  \"{col.Name}\": mean={col.Mean:F4}, median={col.Median:F4}, min={col.Min:F4}, max={col.Max:F4}");
    ...
    private async Task<string> SummarizeResultAsync(string question, object result)
    {
        ...
        if (resultJson.Length > 2000) resultJson = resultJson[..2000] + "...";
    ...
    private async Task CreateTableViewAsync(string readExpr)
    {
        try
        {
            ...
            if (_verbose) Console.WriteLine($"[SQL] Created view 't' from {readExpr}");
        }
        catch (Exception ex)
        {
            if (_verbose) Console.WriteLine($"[SQL] Failed to create view: {ex.Message}");
            throw;
        }
    }
    ...
    /// </summary>
    public async Task<NovelPatternAnalysis?> AnalyzeNovelPatternAsync(string columnName, List<string> examples,
        string? basicRegex)
    {
        ...
        var jsonSchema =
            """{"pattern_name": "short name", "description": "description", "regex": "regex pattern", "is_identifier": true/false, "is_sensitive": true/false, "validation_rules": ["rule1"]}""";
        var prompt = $"""
        ...
        try
        {
            ...
            return new NovelPatternAnalysis
            {
                ...
                ValidationRules = root.TryGetProperty("validation_rules", out var rules) &&
                                  rules.ValueKind == JsonValueKind.Array
                    ? rules.EnumerateArray().Select(r => r.GetString() ?? "").Where(r => !string.IsNullOrEmpty(r))
                        .ToList()
                    : new List<string>()
            };
        }
    ...
    /// </summary>
    public async Task<List<string>> GeneratePatternExamplesAsync(string patternDescription, string? regex,
        int count = 10)
    {
        var prompt = $"""
