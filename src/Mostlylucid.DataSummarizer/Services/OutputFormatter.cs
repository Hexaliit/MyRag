...
public static class OutputFormatter
{
    ...
    public static string FormatMarkdown(DataSummaryReport report, OutputProfileConfig profile, string fileName)
    {
        ...
        sb.AppendLine(
            $"| Numeric Columns | {report.Profile.Columns.Count(c => c.InferredType == ColumnType.Numeric)} |");
        sb.AppendLine(
            $"| Categorical Columns | {report.Profile.Columns.Count(c => c.InferredType == ColumnType.Categorical)} |");
        sb.AppendLine(
            $"| DateTime Columns | {report.Profile.Columns.Count(c => c.InferredType == ColumnType.DateTime)} |");
        ...
        if (mdSettings.IncludeColumnDetails)
        {
            ...
            foreach (var col in report.Profile.Columns.Take(50))
            {
                ...
                sb.AppendLine(
                    $"| {col.Name} | {col.InferredType} | {role} | {col.NullPercent:F1}% | {col.UniqueCount:N0} | {notes} |");
            }
    ...
    public static void WriteConsole(DataSummaryReport report, OutputProfileConfig profile, bool verbose = false)
    {
        ...
        if (console.ShowSummary)
        {
            ...
            summaryTable.AddRow("Numeric",
                report.Profile.Columns.Count(c => c.InferredType == ColumnType.Numeric).ToString());
            summaryTable.AddRow("Categorical",
                report.Profile.Columns.Count(c => c.InferredType == ColumnType.Categorical).ToString());
            summaryTable.AddRow("DateTime",
                report.Profile.Columns.Count(c => c.InferredType == ColumnType.DateTime).ToString());
        ...
        if (console.ShowAlerts && report.Profile.Alerts.Count > 0)
        {
            ...
            foreach (var alert in alertsToShow)
            {
                ...
                AnsiConsole.MarkupLine(
                    $"[{color}]{alert.Severity}:[/] [{color}]{alert.Type}[/] - {Markup.Escape(alert.Message)}");
                if (!string.IsNullOrEmpty(alert.Column))
        ...
        if (console.ShowInsights && report.Profile.Insights.Count > 0)
        {
            ...
            if (report.Profile.Insights.Count > console.MaxInsights)
                AnsiConsole.MarkupLine(
                    $"[dim]...and {report.Profile.Insights.Count - console.MaxInsights} more insights[/]");
            AnsiConsole.WriteLine();
        }
    ...
    private static List<Recommendation> BuildRecommendations(DataSummaryReport report)
    {
        ...
        if (highNullCols.Count > 0)
            recs.Add(new Recommendation
            {
                ...
                Sql =
                    $"SELECT {string.Join(", ", highNullCols.Take(3).Select(c => $"COUNT(*) FILTER (WHERE \"{c.Name}\" IS NULL) AS {c.Name}_nulls"))} FROM data"
            });
        ...
        var highCardCat = p.Columns.Where(c => c.InferredType == ColumnType.Categorical && c.UniquePercent > 50)
            .ToList();
        ...
        var idCols = p.Columns.Where(c => c.SemanticRole == SemanticRole.Identifier || c.InferredType == ColumnType.Id)
            .ToList();
        if (idCols.Count > 0)
            recs.Add(new Recommendation
            {
        ...
        var skewedCols = p.Columns.Where(c => c.InferredType == ColumnType.Numeric && Math.Abs(c.Skewness ?? 0) > 2)
            .ToList();
    ...
    private static List<SuggestedQuery> BuildSuggestedQueries(DataProfile profile)
    {
        ...
        if (nullCols.Count > 0)
        {
            var nullChecks = string.Join(", ",
                nullCols.Select(c => $"SUM(CASE WHEN \"{c.Name}\" IS NULL THEN 1 ELSE 0 END) AS \"{c.Name}_nulls\""));
            queries.Add(new SuggestedQuery
            {
        ...
        if (numericCols.Count > 0)
        {
            var stats = string.Join(", ",
                numericCols.Select(c =>
                    $"AVG(\"{c.Name}\") AS \"{c.Name}_avg\", STDDEV(\"{c.Name}\") AS \"{c.Name}_std\""));
            queries.Add(new SuggestedQuery
            {
        ...
        var catCols = profile.Columns.Where(c => c.InferredType == ColumnType.Categorical && c.UniqueCount < 20).Take(2)
            .ToList();
        foreach (var col in catCols)
            queries.Add(new SuggestedQuery
            {
                ...
                Sql =
                    $"SELECT \"{col.Name}\", COUNT(*) as count, ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER(), 2) as pct FROM {tableName} GROUP BY \"{col.Name}\" ORDER BY count DESC LIMIT 10"
            });
        ...
        foreach (var col in dateCols)
            queries.Add(new SuggestedQuery
            {
                ...
                Sql =
                    $"SELECT MIN(\"{col.Name}\") as earliest, MAX(\"{col.Name}\") as latest, DATEDIFF('day', MIN(\"{col.Name}\"), MAX(\"{col.Name}\")) as span_days FROM {tableName}"
            });

        // Outlier detection
        var outlierCol =
            profile.Columns.FirstOrDefault(c => c.InferredType == ColumnType.Numeric && c.OutlierCount > 0);
        if (outlierCol != null)
            queries.Add(new SuggestedQuery
            {
                Purpose = $"Potential outliers in '{outlierCol.Name}'",
                Sql =
                    $"WITH stats AS (SELECT PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY \"{outlierCol.Name}\") AS q1, PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY \"{outlierCol.Name}\") AS q3 FROM {tableName}) SELECT * FROM {tableName}, stats WHERE \"{outlierCol.Name}\" < q1 - 1.5*(q3-q1) OR \"{outlierCol.Name}\" > q3 + 1.5*(q3-q1) LIMIT 20"
            });
        ...
        if (profile.Target != null)
        {
            ...
            var featureCols = profile.Columns.Where(c => c.Name != targetCol && c.InferredType == ColumnType.Numeric)
                .Take(3).ToList();
            if (featureCols.Count > 0)
            {
                var corrChecks = string.Join(", ",
                    featureCols.Select(c => $"CORR(\"{c.Name}\", \"{targetCol}\") AS \"{c.Name}_corr\""));
                queries.Add(new SuggestedQuery
                {
    ...
    private static List<DataDictionaryEntry> BuildDataDictionary(DataProfile profile)
    {
        return profile.Columns.Select(c => new DataDictionaryEntry
        {
            ...
            Stats = c.InferredType == ColumnType.Numeric
                ? new NumericStats
                {
                    Min = c.Min,
                    Max = c.Max,
                ...
                : null,
            DateRange = c.InferredType == ColumnType.DateTime
                ? new DateRange
                {
    ...
    private static QualityIssues BuildQualityIssues(DataProfile profile)
    {
        ...
        {
            HighNullColumns = profile.Columns.Where(c => c.NullPercent > 20).Select(c => new ColumnIssue
                { Name = c.Name, Value = c.NullPercent, Detail = $"{c.NullPercent:F1}% null" }).ToList(),
            ConstantColumns = profile.Columns.Where(c => c.UniqueCount <= 1)
                .Select(c => new ColumnIssue { Name = c.Name, Detail = "constant value" }).ToList(),
            HighCardinalityColumns = profile.Columns.Where(c => c.UniquePercent > 90).Select(c => new ColumnIssue
                { Name = c.Name, Value = c.UniquePercent, Detail = $"{c.UniquePercent:F1}% unique" }).ToList(),
            OutlierColumns = profile.Columns.Where(c => c.OutlierCount > profile.RowCount * 0.01).Select(c =>
                    new ColumnIssue { Name = c.Name, Value = c.OutlierCount, Detail = $"{c.OutlierCount} outliers" })
                .ToList(),
            SkewedColumns = profile.Columns.Where(c => Math.Abs(c.Skewness ?? 0) > 2).Select(c =>
                new ColumnIssue { Name = c.Name, Value = c.Skewness, Detail = $"skewness {c.Skewness:F2}" }).ToList(),
            PotentialIdColumns = profile.Columns
                .Where(c => c.SemanticRole == SemanticRole.Identifier || c.InferredType == ColumnType.Id)
                .Select(c => new ColumnIssue { Name = c.Name, Detail = "likely identifier" }).ToList()
        };
    }
    ...
    private static LlmProfileSummary BuildProfileSummary(DataProfile profile)
    {
        return new LlmProfileSummary
        {
            ...
            Columns = profile.Columns.Select(c => new LlmColumnSummary
            {
                ...
                TopValues = c.TopValues?.Take(5)
                    .Select(v => new TopValue { Value = v.Value, Count = v.Count, Percent = v.Percent }).ToList()
            }).ToList()
        };
    }
