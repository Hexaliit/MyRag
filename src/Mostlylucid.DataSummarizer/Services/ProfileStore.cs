...
public class ProfileStore
{
    ...
    public StoredProfileInfo Store(DataProfile profile, string? contentHash = null, SegmentInfo? segment = null)
    {
        ...
        var (hash, size) = contentHash != null
            ? (contentHash,
                File.Exists(profile.SourcePath) ? new FileInfo(profile.SourcePath).Length : profile.RowCount)
        ...
        double maxDistance = 0.5)
    {
        ...
        foreach (var (id, info) in _index.Profiles)
        {
            ...
            var distance = ComputeEuclideanDistance(centroid.Vector, info.CentroidVector);
            if (distance <= maxDistance) results.Add((info, distance));
        }
    ...
    public StoredProfileInfo? FindByContentHash(string filePath)
    {
        ...
        if (_index.ByContentHash.TryGetValue(hash, out var id)) return _index.Profiles.GetValueOrDefault(id);
        return null;
    }
    ...
    /// </summary>
    public int PruneOldProfiles(int keepPerSchema = 5)
    {
        return Prune(keepPerSchema);
    }
    ...
    private static (string Hash, long Size) ComputeStreamingHash(string filePath)
    {
        ...
        using var stream = new FileStream(
            ...
            FileShare.Read,
            HashBufferSize,
            false); // Sync is faster for sequential reads
    ...
    /// </summary>
    public static async Task<(string Hash, long Size)> ComputeFileHashAsync(string filePath,
        CancellationToken ct = default)
    {
        ...
        int bytesRead;
        while ((bytesRead = await stream.ReadAsync(buffer, ct)) > 0) hasher.Append(buffer.AsSpan(0, bytesRead));
    ...
    public static (string Hash, long Size) ComputeSourceHash(DataProfile profile)
    {
        ...
        if (File.Exists(profile.SourcePath)) return ComputeFileHashWithSize(profile.SourcePath);
    ...
    public static StatisticalSignature ComputeStatisticalSignature(DataProfile profile)
    {
        ...
        if (numericCols.Count > 0)
        {
            ...
            sig.NumericStdDevs = numericCols.Where(c => c.StdDev.HasValue).Select(c => NormalizeStat(c.StdDev!.Value))
                .ToList();
        }
        ...
        if (catCols.Count > 0) sig.CategoricalCardinalities = catCols.Select(c => (int)c.UniqueCount).ToList();
    ...
    private static string GetRowCountBucket(long rowCount)
    {
