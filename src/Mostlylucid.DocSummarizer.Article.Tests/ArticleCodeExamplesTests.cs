...
public class ArticleCodeExamplesTests
{
    ...
    public void SalienceFiltering_ExampleWorks()
    {
        ...
        var extraction = new ExtractionResult
        {
            AllSegments = Enumerable.Range(0, 100)
                .Select(i =>
                {
                    var s = CreateSegment("doc", $"s{i}", $"Segment {i}", SegmentType.Sentence, i, i * 10,
                        (i + 1) * 10);
    ...
    {
        var segment = CreateSegmentWithPageAndSection("doc", "s42", "Important text here", SegmentType.Sentence, 0, 100,
            119, 3, "Results", "Chapter 2 > Results");
    ...
    public void ExtractionResult_GetHighlightedText_Works()
    {
        ...
        var highlighted = result.GetHighlightedText(originalDoc, segment.Id, 15);
    ...
    public class SimpleRagService
    {
        ...
        public void Index(ExtractionResult extraction, string docId)
        {
            foreach (var segment in extraction.AllSegments)
            {
                ...
                if (segment.Embedding != null) _index.Add((id, segment.Embedding));
            }
        }
    ...
    private static Segment CreateSegment(string docId, string _, string text, SegmentType type, int index,
    ...
    private static Segment CreateSegmentWithPage(string docId, string _, string text, SegmentType type, int index,
        int startChar, int endChar, int pageNumber)
    {
    ...
    private static Segment CreateSegmentWithPageAndSection(string docId, string _, string text, SegmentType type,
        int index, int startChar, int endChar, int pageNumber, string sectionTitle, string headingPath)
    {
        var segment = new Segment(docId, text, type, index, startChar, endChar)
        {
            PageNumber = pageNumber,
