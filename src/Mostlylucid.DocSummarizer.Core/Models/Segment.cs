...
public class Segment
{
    ...
    {
        return ContentHasher.ComputeHash(text);
    }
    ...
    /// </summary>
    public CitationInfo ToCitation()
    {
...
public class ExtractionResult
{
    ...
    {
        return SegmentsById.TryGetValue(segmentId, out var segment) ? segment : null;
    }
    ...
    {
        return SegmentsByIndex.TryGetValue(index, out var segment) ? segment : null;
    }
    ...
    {
        return AllSegments.FirstOrDefault(s => charPosition >= s.StartChar && charPosition < s.EndChar);
    }
    ...
    {
        return AllSegments.Where(s => s.PageNumber == pageNumber).OrderBy(s => s.Index);
    }
    ...
    {
        return AllSegments.Where(s => s.StartChar < endChar && s.EndChar > startChar).OrderBy(s => s.StartChar);
    }
    ...
    {
        return SegmentsById.TryGetValue(segmentId, out segment);
    }
    ...
    public HighlightedText? GetHighlightedText(string originalDocument, string segmentId, int contextChars = 0)
    {
        ...
        return new HighlightedText(
            originalDocument[contextStart..contextEnd],
            location.StartChar - contextStart,
            location.EndChar - contextStart,
            contextChars > 0 ? originalDocument[contextStart..location.StartChar] : "",
            originalDocument[location.StartChar..location.EndChar],
            contextChars > 0 ? originalDocument[location.EndChar..contextEnd] : "",
            location
        );
    }
}
...
)
{
    ...
    {
        return
            $"{WebUtility.HtmlEncode(BeforeContext)}<span class=\"{highlightClass}\">{WebUtility.HtmlEncode(SegmentText)}</span>{WebUtility.HtmlEncode(AfterContext)}";
    }
    ...
    {
        return $"{BeforeContext}**{SegmentText}**{AfterContext}";
    }
}
