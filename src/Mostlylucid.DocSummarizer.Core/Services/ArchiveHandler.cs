...
public static partial class ArchiveHandler
{
        ...
        try
        {
            ...
            if (totalUncompressed > maxSize)
                return new ArchiveInfo
                {
                    IsValid = false,
                    Error =
                        $"Uncompressed size too large ({totalUncompressed / (1024 * 1024):F1} MB > {maxSize / (1024 * 1024)} MB limit)"
                };
        ...
        CancellationToken ct = default)
    {
        ...
        if (entry == null) throw new InvalidOperationException($"Entry not found: {info.MainFileName}");
        ...
        if (ext is ".html" or ".htm" or ".xhtml") content = ConvertHtmlToMarkdown(content, info.IsGutenberg);
    ...
    private static string ConvertHtmlToMarkdown(string html, bool isGutenberg)
    {
        ...
        if (isGutenberg) html = RemoveGutenbergBoilerplate(html);
    ...
    private static string RemoveGutenbergBoilerplate(string html)
    {
        ...
        if (startMatch.Success) html = html[(startMatch.Index + startMatch.Length)..];

        // Remove everything after "*** END OF" marker
        var endMatch = GutenbergEndRegex().Match(html);
        if (endMatch.Success) html = html[..endMatch.Index];
    ...
    private static int GetExtensionPriority(string ext)
    {
    ...
    [GeneratedRegex(@"<[^>]*(id|class)\s*=\s*[""']pg-(header|footer)[""'][^>]*>.*?</[^>]+>",
