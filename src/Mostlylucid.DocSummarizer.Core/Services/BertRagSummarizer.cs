...
public class BertRagSummarizer : IDisposable, IAsyncDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (extraction.AllSegments.Count == 0) return (extraction, new List<Segment>());

        // === Phase 2: Retrieve ===
        VerboseHelper.Log(_verbose, "[bold cyan]Phase 2: Retrieval[/]");
        var retrieved = await RetrieveAsync(extraction, focusQuery, ct);

        if (_verbose) VerboseHelper.Log(_verbose, $"[dim]Retrieved {retrieved.Count} segments for synthesis[/]");
        ...
        CancellationToken ct = default)
    {
        ...
        if (retrieved.Count == 0) return CreateEmptySummary(docId, "No segments found in document");

        // === Phase 3: Synthesize (using current template) ===
        VerboseHelper.Log(_verbose,
            $"[bold cyan]Phase 3: Synthesis with template '{VerboseHelper.Escape(Template.Name)}'[/]");
    ...
    private string GeneratePreRetrievalCacheKey(string contentHash, string? focusQuery, string modelName)
    {
        ...
        var retrievalConfig =
            $"{_retrievalConfig.TopK}_{_retrievalConfig.Alpha}_{_retrievalConfig.UseRRF}_{_retrievalConfig.UseHybridSearch}_{_retrievalConfig.AdaptiveTopK}_{_retrievalConfig.MaxTopK}";
        ...
        return
            $"{PipelineVersion}_{contentHash}_{queryHash}_{templateHash}_{retrievalHash}_{embeddingHash}_{modelHash}";
    }
        ...
        CancellationToken ct = default)
    {
        ...
        if (extraction.AllSegments.Count == 0) return CreateEmptySummary(docId, "No segments found in document");
        ...
        var trace = new SummarizationTrace(
            ...
            stopwatch.Elapsed,
            coverage,
            1.0 // BERT-RAG always has perfect citations
        );
    ...
    private int CalculateAdaptiveTopK(int totalSegments, ContentType contentType)
    {
        ...
        if (_verbose && adaptiveTopK != _retrievalConfig.TopK)
        {
            ...
            VerboseHelper.Log(
                $"[dim]Adaptive TopK: {adaptiveTopK} (base {_retrievalConfig.TopK}, {totalSegments} segments, {contentLabel})[/]");
        }
        ...
        CancellationToken ct)
    {
        ...
        if (_retrievalConfig.UseRRF)
        {
            if (_retrievalConfig.UseHybridSearch)
            {
                ...
                VerboseHelper.Log(_verbose,
                    $"[dim]Using Hybrid RRF: Dense + BM25 + Salience (k={_retrievalConfig.RrfK}, topK={effectiveTopK})[/]");
            }
            else
            {
                ...
                VerboseHelper.Log(_verbose,
                    $"[dim]Using RRF: Dense + Salience (k={_retrievalConfig.RrfK}, topK={effectiveTopK})[/]");
            }
        }
        else
        {
        ...
        if (_verbose)
        {
            ...
            VerboseHelper.Log(_verbose,
                $"[dim]Top by retrieval: {topByRetrieval.Count}, fallback added: {result.Count - topByRetrieval.Count}[/]");
        }
        ...
        CancellationToken ct)
    {
        ...
        var synthesisSegments = FilterForSynthesis(retrieved, true);
        ...
        if (string.IsNullOrEmpty(documentTitle)) documentTitle = ExtractTitleFromContent(synthesisSegments, docId);
        ...
        if (ollamaAvailable)
        {
            var rawSummary = await _ollama.GenerateAsync(synthesisPrompt);
    ...
    private static string? ExtractTitleFromContent(List<Segment> segments, string docId)
    {
        ...
        if (string.IsNullOrEmpty(title)) return FallbackToFilename(docId);
