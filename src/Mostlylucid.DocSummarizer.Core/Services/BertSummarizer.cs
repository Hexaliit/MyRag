...
public class BertSummarizer : IDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (parsedDoc.SentenceCount == 0) return CreateEmptySummary("No sentences found in document");

        VerboseHelper.Log(_verbose,
            $"[dim]Found {parsedDoc.SentenceCount} sentences in {parsedDoc.Sections.Count} sections[/]");
    ...
    private static float[] CalculateCentroid(List<SentenceInfo> sentences)
    {
        ...
        foreach (var sentence in sentences)
        {
            ...
            for (var i = 0; i < dim; i++) centroid[i] += sentence.Embedding[i];
            count++;
        }
        ...
        TimeSpan elapsed)
    {
        ...
        foreach (var sentence in selectedSentences.Take(5)) executiveParts.Add($"{sentence.Text} {sentence.Citation}");
        ...
        var trace = new SummarizationTrace(
            ...
            elapsed,
            (double)selectedSentences.Count / parsedDoc.SentenceCount,
            1.0 // BERT extraction always has perfect citations
        );
        ...
        return new DocumentSummary(
            executiveSummary,
            topicSummaries,
            new List<string>(), // BERT extraction doesn't generate questions
            trace,
            entities
        );
    }
