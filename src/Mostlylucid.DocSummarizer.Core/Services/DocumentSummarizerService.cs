...
public class DocumentSummarizerService : IDocumentSummarizer, IDisposable
{
    ...
    public async Task<DocumentSummary> SummarizeMarkdownAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        using var activity = ActivitySource.StartActivity("SummarizeMarkdown");
        ...
        try
        {
            ...
            var result =
                await _bertRag.SummarizeAsync(docId, markdown, focusQuery, ContentType.Unknown, cancellationToken);
    ...
    public async Task<DocumentSummary> SummarizeFileAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        using var activity = ActivitySource.StartActivity("SummarizeFile");
        ...
        try
        {
            ...
            switch (ext)
            {
                ...
                case ".htm":
                    ...
                    progress.WriteInfo("FileLoad", $"Converted HTML to {markdown.Length:N0} characters",
                        sw.ElapsedMilliseconds);
                    break;
    ...
    public async Task<DocumentSummary> SummarizeUrlAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        using var activity = ActivitySource.StartActivity("SummarizeUrl");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result =
                await _bertRag.SummarizeAsync(docId, markdown, question, ContentType.Unknown, cancellationToken);
            ...
            return new QueryAnswer(
                ...
                ConfidenceLevel.Medium, // Could be computed from scores
                evidence,
                question);
        }
        catch (Exception ex)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        progress.WriteCompleted($"Extracted {extraction.AllSegments.Count} segments", sw.ElapsedMilliseconds,
    ...
    public async Task<ServiceAvailability> CheckServicesAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (ollamaAvailable) ollamaModel = _config.Ollama.Model;
        }
        catch
        {
