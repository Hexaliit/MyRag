...
public class InMemoryVectorStore : IVectorStore
{
    ...
    public Task UpsertSegmentsAsync(string collectionName, IEnumerable<Segment> segments,
    ...
    public Task<List<Segment>> GetDocumentSegmentsAsync(string collectionName, string docHash,
        CancellationToken ct = default)
    {
        if (!_collections.TryGetValue(collectionName, out var segments))
            return Task.FromResult(new List<Segment>());
        ...
        CancellationToken ct = default)
    {
        ...
        if (_verbose && summary != null)
            Console.WriteLine(
                $"[InMemoryVectorStore] Summary cache hit for evidence '{evidenceHash[..Math.Min(16, evidenceHash.Length)]}'");

        return Task.FromResult(summary);
    }

    public Task CacheSummaryAsync(string collectionName, string evidenceHash, DocumentSummary summary,
        CancellationToken ct = default)
    {
        var fullKey = $"{collectionName}:{evidenceHash}";
        ...
        if (_verbose)
            Console.WriteLine(
                $"[InMemoryVectorStore] Cached summary for evidence '{evidenceHash[..Math.Min(16, evidenceHash.Length)]}'");
