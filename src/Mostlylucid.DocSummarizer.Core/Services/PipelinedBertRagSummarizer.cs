...
public class PipelinedBertRagSummarizer : IDisposable
{
    ...
    public SummaryTemplate Template { get; }
    ...
    public void OnChunkReady(string docId, int chunkIndex, string markdown)
    {
        ...
        if (string.IsNullOrWhiteSpace(cleanMarkdown))
        {
            if (_verbose && !ProgressService.IsInInteractiveContext)
                VerboseHelper.Log(_verbose,
                    $"[yellow]Chunk {chunkIndex}: empty text returned from converter, skipping[/]");
            return;
        }
        ...
        if (segments.Count == 0)
        {
            if (_verbose && !ProgressService.IsInInteractiveContext)
                VerboseHelper.Log(_verbose,
                    $"[yellow]Chunk {chunkIndex}: no segments parsed after sanitization, retrying with original text[/]");
            segments = ParseChunkToSegments(docId, markdown, chunkIndex);
            if (segments.Count == 0)
            {
                if (_verbose && !ProgressService.IsInInteractiveContext)
                    VerboseHelper.Log(_verbose,
                        $"[yellow]Chunk {chunkIndex}: still no segments parsed, skipping chunk[/]");
                return;
            }
        }
        ...
        var inTail = ChunksReceived >= tailStart;
        ...
        var isBootstrap = ChunksReceived < _bootstrapChunks;
        var budgetAvailable = effectiveBudget > 0;
        var shouldTrickle = !budgetAvailable && ChunksReceived % _trickleEveryChunks == 0;
        ...
        foreach (var segment in segments) _accumulatedSegments.Add(segment);
        foreach (var segment in toEmbed) _pendingEmbedding.Enqueue(segment);
        ...
        if (_verbose)
        {
            ...
            if (!ProgressService.IsInInteractiveContext)
                VerboseHelper.Log(
                    $"[dim]Chunk {chunkIndex}: queued {toEmbed.Count}/{segments.Count} ({pct:P0}) for embedding, pending={_pendingEmbedding.Count}, embedded={_embeddedBudgetUsed}/{_globalEmbedBudget}[/]");
        }

        // Detect content type from first chunk
        if (ChunksReceived == 1 && segments.Count > 0)
        {
    ...
    private List<Segment> ParseChunkToSegments(string docId, string markdown, int chunkIndex)
    {
        ...
        foreach (var section in parsedDoc.Sections)
        {
            ...
            foreach (var sentenceInfo in section.Sentences)
            {
                ...
                var idx = Interlocked.Increment(ref _segmentCounter);
                var segment = new Segment(docId, sentenceInfo.Text, SegmentType.Sentence, idx, 0,
                    sentenceInfo.Text.Length)
                {
                    SectionTitle = section.Heading,
                    HeadingPath = currentHeadingPath,
                    HeadingLevel = section.Level,
    ...
    private float[] GetCurrentCentroid()
    {
        ...
        for (var i = 0; i < dim; i++) centroid[i] /= embedded.Count;
    ...
    {
        var typeBoost = segment.Type switch
        {
    ...
    private async Task BackgroundEmbeddingLoop(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            // Collect a batch
            batch.Clear();
            while (batch.Count < batchSize && _pendingEmbedding.TryDequeue(out var segment)) batch.Add(segment);
            ...
            if (batch.Count > 0)
            {
                ...
                for (var i = 0; i < batch.Count; i++) batch[i].Embedding = embeddings[i];
                _embeddedBudgetUsed += batch.Count;

                if (_verbose && batch.Count > 0)
                    if (!ProgressService.IsInInteractiveContext)
                        VerboseHelper.Log(
                            $"[dim]Background embedded {batch.Count} segments (budget {_embeddedBudgetUsed}/{_globalEmbedBudget})[/]");
            }
            else
            {
        ...
        CancellationToken ct = default)
    {
        ...
        if (_verbose)
            if (!ProgressService.IsInInteractiveContext)
                VerboseHelper.Log(_verbose,
                    $"[bold cyan]Finalizing: {_accumulatedSegments.Count} segments from {ChunksReceived} chunks[/]");
        ...
        if (_backgroundEmbeddingTask != null)
            try
            {
                await _backgroundEmbeddingTask;
            }
        ...
        if (segments.Count == 0) return CreateEmptySummary(docId, "No segments embedded from document");
        ...
        var trace = new SummarizationTrace(
            ...
            stopwatch.Elapsed,
            (double)retrieved.Count / segments.Count,
            1.0);
    ...
    private async Task WaitForPendingEmbeddingsAsync(CancellationToken ct)
    {
        ...
        if (!_pendingEmbedding.IsEmpty && _verbose)
            VerboseHelper.Log(_verbose,
                $"[yellow]Warning: {_pendingEmbedding.Count} segments still pending after {maxWait.TotalSeconds}s[/]");
    }
        ...
        CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(focusQuery)) return topBySalience.Take(_retrievalConfig.TopK).ToList();
        ...
        for (var i = 0; i < byQuerySim.Count; i++)
        {
        ...
        CancellationToken ct)
    {
        ...
        var rawSummary = await _ollama.GenerateAsync(prompt);
        ...
        return new DocumentSummary(
            ...
            topicSummaries,
            new List<string>(),
            new SummarizationTrace(docId, SegmentCount, retrieved.Count, new List<string>(), TimeSpan.Zero, coverage,
                0),
            entities);
    }
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            facts = await _ollama.GenerateAsync(factPrompt, 0.1);
        }
        catch
        {
        ...
        try
        {
            var corrected = await _ollama.GenerateAsync(checkPrompt, 0.1);
    ...
    private static ExtractedEntities ExtractEntities(List<Segment> segments, ContentType contentType)
    {
        ...
        foreach (var (name, count) in grouped.OrderByDescending(g => g.Count))
        {
            ...
            var hasHonorific = words.Length > 1 && honorifics.Contains(words[0]);
        ...
        return new ExtractedEntities(final, new List<string>(), new List<string>(), new List<string>(),
            new List<string>());
        ...
        bool IsContinuation(string token)
        {
            ...
            if (token.Length > 1 && char.IsUpper(token[0]) &&
                token.Skip(1).All(c => char.IsLower(c) || c == '\'')) return true;
            return false;
        }
    ...
    private static bool LooksLikeCodeContent(string text)
    {
        ...
        {
            "```", "function", "class", "void", "int ", "return", "if (", "for (", "while (",
    ...
    {
        return new DocumentSummary(message, new List<TopicSummary>(), new List<string>(),
