...
public class ProgressService
{
    ...
    /// </summary>
    public static bool ShouldShowVerbose(bool verbose)
    {
        return verbose;
    }
    ...
    {
        if (verbose) VerboseHelper.Log(message);
    }
    ...
    {
        if (_verbose) Console.Error.WriteLine($"[INFO] {VerboseHelper.Escape(message)}");
    }
    ...
    {
        if (_verbose) Console.Error.WriteLine($"[SUCCESS] {VerboseHelper.Escape(message)}");
    }
    ...
    {
        if (_verbose) Console.Error.WriteLine($"[WARNING] {VerboseHelper.Escape(message)}");
    }
    ...
    {
        if (_verbose) Console.Error.WriteLine($"[STATUS] {statusMessage}");
        await action();
    }
