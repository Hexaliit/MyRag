...
public class QdrantVectorStore : IVectorStore
{
    ...
    public QdrantVectorStore(string host = "localhost", int port = 6334, string? apiKey = null, int vectorSize = 384,
        bool verbose = false, bool deleteOnDispose = false)
    {
    ...
    public async Task InitializeAsync(string collectionName, int vectorSize, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!exists)
            {
                ...
                if (_verbose)
                    VerboseHelper.Log(
                        $"[dim]Created Qdrant collection '{VerboseHelper.Escape(collectionName)}' (dim={vectorSize})[/]");
            }
            else if (_verbose)
            {
        ...
        CancellationToken ct = default)
    {
        ...
        for (var i = 0; i < points.Count; i += batchSize)
        {
            ...
            if (_verbose && points.Count > batchSize)
                VerboseHelper.Log(
                    $"[dim]Upserted batch {i / batchSize + 1}/{(points.Count + batchSize - 1) / batchSize}[/]");
        }
        ...
        CancellationToken ct = default)
    {
        ...
        if (_verbose)
            VerboseHelper.Log(
                $"[dim]GetDocumentSegmentsAsync: collection='{VerboseHelper.Escape(collectionName)}', docHash='{VerboseHelper.Escape(docHash)}'[/]");
    ...
    public async Task DeleteDocumentAsync(string collectionName, string docId, CancellationToken ct = default)
    {
        ...
        if (_verbose)
            VerboseHelper.Log(
                $"[dim]Deleted document by hash from Qdrant collection '{VerboseHelper.Escape(collectionName)}'[/]");
    }
    ...
    public async Task<DocumentSummary?> GetCachedSummaryAsync(string collectionName, string cacheKey,
        CancellationToken ct = default)
    {
        try
        {
    ...
    public async Task CacheSummaryAsync(string collectionName, string cacheKey, DocumentSummary summary,
        CancellationToken ct = default)
    {
        try
        {
