...
public class ServiceFeatures
{
    ...
    public ServiceFeatures(DetectedServices services)
    {
        _services = services;
    }
...
public static class ServiceDetector
{
    ...
    public static async Task<DetectedServices> DetectAndDisplayAsync(DocSummarizerConfig config, bool verbose = false,
        bool? forceGpu = null)
    {
        ...
        if (forceGpu.HasValue && result.DoclingAvailable) result.DoclingHasGpu = forceGpu.Value;
        ...
        if (!result.OllamaAvailable) tips.Add("Tip: Start Ollama for LLM-enhanced summaries: ollama serve");

        if (!result.DoclingAvailable)
            tips.Add(
                "Tip: Start Docling for PDF/DOCX support: docker run -p 5001:5001 quay.io/docling-project/docling-serve");
    ...
    private static void DisplayConfigurationReasoning(DetectedServices services)
    {
        ...
        {
            VerboseHelper.Log(
                "  [yellow]Summarization:[/] BERT only [dim](no LLM detected = extractive summarization, still works!)[/]");
        }
