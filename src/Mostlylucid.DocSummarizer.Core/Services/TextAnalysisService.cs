...
public class TextAnalysisService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var termScores = baseTerms.ToDictionary(t => t.term, t => (t.score, entityType: (string?)null));
    ...
    private string RemoveHedgingPhrases(string text)
    {
        ...
        foreach (var phrase in hedgingPhrases) result = Regex.Replace(result, phrase, "", RegexOptions.IgnoreCase);
    ...
    public HashSet<string> ExtractNGrams(string text, int n = 2)
    {
        ...
        for (var i = 0; i <= normalized.Length - n; i++) ngrams.Add(normalized.Substring(i, n));
