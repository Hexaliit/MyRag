...
public static class CollectionDetector
{
    ...
    public static CollectionInfo Analyze(ParsedDocument document, string? title = null)
    {
        ...
        if (isChapteredNovel) confidence *= 0.2; // Reduce to 20% - almost certainly not a collection
    ...
    private static ParsedSection? FindTocSection(List<ParsedSection> sections)
    {
        foreach (var section in sections)
        {
            ...
            if (TocPatterns.Any(p => headingLower.Contains(p))) return section;
        }
    ...
    private static double CalculateConfidence(int h1Count, int tocEntryCount, bool hasCollectionTitle,
        bool isShakespeare)
    {
        var confidence = 0.0;
