...
public static class StopwordLists
{
    ...
    private static readonly Lazy<HashSet<string>>
        _placeIndicators = new(() => LoadEmbeddedList("place-indicators.txt"));
    ...
    private static HashSet<string> LoadStopwords()
    {
        ...
        try
        {
            ...
            foreach (var word in englishStopwords) set.Add(word);
        }
        catch
        {
    ...
    private static void AddEssentialStopwords(HashSet<string> set)
    {
        ...
        {
            "and", "but", "or", "nor", "so", "for", "yet", "because",
    ...
    private static void AddCustomStopwords(HashSet<string> set)
    {
        ...
        var gutenberg = new[]
        {
            "project", "gutenberg", "foundation", "archive",
        ...
        var geographic = new[]
        {
            "island", "river", "lake", "mountain", "valley",
        ...
        var verbs = new[]
        {
            "let", "could", "would", "should", "must", "shall",
            "will", "may", "might", "can"
        };
    ...
    private static HashSet<string> LoadEmbeddedList(string fileName)
    {
        ...
        try
        {
            ...
            while (reader.ReadLine() is { } line)
            {
                ...
                if (!string.IsNullOrEmpty(trimmed) && !trimmed.StartsWith('#')) set.Add(trimmed);
            }
        }
        catch
        {
    ...
    private static HashSet<string> GetDefaultList(string fileName)
    {
