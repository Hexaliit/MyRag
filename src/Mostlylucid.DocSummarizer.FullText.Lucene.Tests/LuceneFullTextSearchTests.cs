...
public class LuceneFullTextSearchTests : IAsyncLifetime
{
    ...
    public Task DisposeAsync()
    {
        ...
        if (Directory.Exists(_indexPath)) Directory.Delete(_indexPath, true);
    ...
    {
        // Arrange
        await _search.IndexDocumentAsync("doc1", "Introduction to Machine Learning",
            "This document covers supervised and unsupervised learning algorithms.");
    ...
    public async Task TitleBoost_RanksHigher()
    {
        ...
        await _search.IndexDocumentAsync("title-match", "Software Architecture Patterns",
            "This document discusses various design approaches for building systems.");
        await _search.IndexDocumentAsync("content-match", "Development Guide",
            "This document explains software architecture in modern applications.");
        ...
        titleMatch.Score.Should().BeGreaterThan(contentMatch.Score,
            "title matches should be boosted higher than content-only matches");
    }
    ...
    public async Task KeywordBoost_RanksHigher()
    {
        ...
        await _search.IndexDocumentAsync("keyword-match", "Search Engine Overview",
            "A general overview of search technologies.", new[] { "elasticsearch", "lucene", "solr" });
        await _search.IndexDocumentAsync("content-match", "Database Technologies",
            "This guide covers elasticsearch and other data storage solutions.");
        ...
        var contentMatch = results.First(r => r.Id == "content-match");
        keywordMatch.Score.Should().BeGreaterThan(contentMatch.Score,
            "keyword matches should be boosted higher than content-only matches");
    }
    ...
    {
        // Arrange
        await _search.IndexDocumentAsync("doc-to-delete", "Temporary Document",
    ...
    {
        // Arrange
        await _search.IndexDocumentAsync("relevant", "Cloud Computing Fundamentals",
            "An introduction to distributed systems and cloud native infrastructure.");
        await _search.IndexDocumentAsync("partial", "Desktop Applications",
            "Building native applications for Windows and macOS.");
    ...
    public async Task FuzzySearch_FindsSimilarTerms()
    {
        ...
        await _search.IndexDocumentAsync("doc1", "Programming Languages",
            "A comprehensive guide to programming paradigms and language design.");
