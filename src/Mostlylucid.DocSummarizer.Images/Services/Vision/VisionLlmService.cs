...
public class VisionLlmService
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var prompt = customPrompt ??
                         "Analyze this image in detail. Describe what you see, including objects, people, setting, mood, colors, and any text visible. Be specific and comprehensive.";
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var prompt =
                $@"You are a query analyzer for image search. Break down the following natural language query into structured filters.
...
Only return valid JSON, nothing else.";
    ...
    private string ExtractCaptionFromResponse(string response)
    {
        ...
        if (match.Success && match.Groups.Count > 1) return SanitizeAndTruncate(match.Groups[1].Value);
...
{
    [JsonPropertyName("filters")] public List<ImageFilter> Filters { get; set; } = [];
}
