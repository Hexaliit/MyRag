...
public class TemplateBenchmarkService : IAsyncDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (templates.Count == 0) throw new ArgumentException("At least one template must be specified");
        ...
        if (_verbose)
        {
            AnsiConsole.MarkupLine(
                $"[green]Extraction complete:[/] {extraction.AllSegments.Count} segments, {retrieved.Count} retrieved");
        ...
        foreach (var template in templates)
        {
            if (_verbose)
                AnsiConsole.MarkupLine(
                    $"[cyan]Synthesizing with template:[/] [yellow]{template.Name}[/] ({template.Description})");
            ...
            try
            {
                ...
                if (_verbose)
                    AnsiConsole.MarkupLine(
                        $"  [green]Done:[/] {wordCount} words in {synthesisStopwatch.Elapsed.TotalSeconds:F2}s");
            }
            catch (Exception ex)
            {
                synthesisStopwatch.Stop();
                ...
                if (_verbose) AnsiConsole.MarkupLine($"  [red]Failed:[/] {ex.Message}");
            }
        }
        ...
        string baseFileName)
    {
        ...
        foreach (var result in results.TemplateResults.Where(r => r.Success))
        {
            ...
            if (_verbose) AnsiConsole.MarkupLine($"[dim]Saved: {filePath}[/]");
        }
        ...
        if (_verbose) AnsiConsole.MarkupLine($"[green]Saved benchmark report: {reportPath}[/]");
    }
    ...
    private static string FormatBenchmarkReport(BenchmarkResults results)
    {
        ...
        foreach (var r in results.TemplateResults)
            if (r.Success)
            {
                ...
                sb.AppendLine(
                    $"| {r.TemplateName} | {r.TargetWords} | {r.ActualWordCount} | {diffStr} | {r.SynthesisTime.TotalSeconds:F2}s |");
            }
            else
            {
