...
public class DocumentTypeDetectorTests
{
    ...
    public async Task ClassifyAsync_ReturnsHeuristicWhenConfidenceHigh()
    {
        ...
        var detector = new DocumentTypeDetector(ollama: null);

        var result = await detector.ClassifyAsync(
            "some content",
            heuristicType: "fiction",
            heuristicConfidence: 0.80,
            heuristicSignals: ["chapter_markers→fiction", "dialogue→fiction"]);
    ...
    {
        var detector = new DocumentTypeDetector(ollama: null) { LlmFallbackThreshold = 0.60 };
    ...
    public void FindSubstantialChunk_FallsBackForShortContent()
    {
        ...
        var chunk = DocumentTypeDetector.FindSubstantialChunk(content, targetLength: 500);
