...
public static partial class BookTypeDetector
{
    ...
    private static void CollectFilenameSignals(string fileName, List<DetectionSignal> signals)
    {
        ...
        if (ext is ".epub" or ".mobi")
            signals.Add(new DetectionSignal { Name = "ebook_extension", Category = "filename", Weight = 3.0, VotedType = Fiction, Reason = $"eBook format ({ext})" });

        // Filename content signals
        if (name.Contains("novel") || name.Contains("story") || name.Contains("tales"))
            signals.Add(new DetectionSignal { Name = "fiction_filename", Category = "filename", Weight = 5.0, VotedType = Fiction, Reason = "Filename suggests fiction" });

        if (name.Contains("paper") || name.Contains("thesis") || name.Contains("dissertation"))
            signals.Add(new DetectionSignal { Name = "academic_filename", Category = "filename", Weight = 5.0, VotedType = Academic, Reason = "Filename suggests academic" });

        if (name.Contains("manual") || name.Contains("guide") || name.Contains("handbook") || name.Contains("docs"))
            signals.Add(new DetectionSignal { Name = "technical_filename", Category = "filename", Weight = 5.0, VotedType = Technical, Reason = "Filename suggests technical" });

        if (name.Contains("complete-works") || name.Contains("collected") || name.Contains("anthology"))
            signals.Add(new DetectionSignal { Name = "anthology_filename", Category = "filename", Weight = 8.0, VotedType = Anthology, Reason = "Filename suggests anthology" });

        if (name.Contains("poems") || name.Contains("poetry") || name.Contains("verses") || name.Contains("sonnets"))
            signals.Add(new DetectionSignal { Name = "poetry_filename", Category = "filename", Weight = 8.0, VotedType = Collection, Reason = "Filename suggests poetry collection" });
        if (name.Contains("stories") || name.Contains("tales") || name.Contains("collection") || name.Contains("essays"))
            signals.Add(new DetectionSignal { Name = "collection_filename", Category = "filename", Weight = 6.0, VotedType = Collection, Reason = "Filename suggests collection" });

        if (name.Contains("play") || name.Contains("drama") || name.Contains("tragedy") || name.Contains("comedy"))
            signals.Add(new DetectionSignal { Name = "play_filename", Category = "filename", Weight = 5.0, VotedType = Play, Reason = "Filename suggests dramatic work" });

        // Gutenberg-style naming: pg12345
        if (GutenbergIdRegex().IsMatch(name))
            signals.Add(new DetectionSignal { Name = "gutenberg_id", Category = "filename", Weight = 2.0, VotedType = Fiction, Reason = "Gutenberg eBook ID pattern" });
    }
    ...
    private static void CollectSizeSignals(int wordCount, long? fileSize, List<DetectionSignal> signals)
    {
        ...
        if (wordCount > 200_000)
            signals.Add(new DetectionSignal { Name = "very_long", Category = "size", Weight = 3.0, VotedType = Anthology, Reason = $"{wordCount:N0} words — likely anthology or collected works" });
        else if (wordCount > 50_000)
            signals.Add(new DetectionSignal { Name = "book_length", Category = "size", Weight = 2.0, VotedType = Fiction, Reason = $"{wordCount:N0} words — novel length" });
        else if (wordCount is > 3_000 and < 15_000)
            signals.Add(new DetectionSignal { Name = "paper_length", Category = "size", Weight = 1.5, VotedType = Academic, Reason = $"{wordCount:N0} words — typical paper length" });
        else if (wordCount < 3_000)
            signals.Add(new DetectionSignal { Name = "short_doc", Category = "size", Weight = 1.0, VotedType = Technical, Reason = $"{wordCount:N0} words — short document" });

        // File size signals (PDFs with images tend to be larger)
        if (fileSize > 50 * 1024 * 1024) // > 50 MB
            signals.Add(new DetectionSignal { Name = "large_file", Category = "size", Weight = 1.5, VotedType = Technical, Reason = "Large file — likely image-heavy technical doc" });
    }
    ...
    private static void CollectMetadataSignals(Dictionary<string, string> metadata, List<DetectionSignal> signals)
    {
        if (metadata.TryGetValue("author", out var author) && !string.IsNullOrWhiteSpace(author))
        {
            ...
            signals.Add(new DetectionSignal { Name = "has_author", Category = "metadata", Weight = 1.0, VotedType = Fiction, Reason = $"Author: {author}" });
        }

        if (metadata.TryGetValue("language", out var lang) && lang.Contains("english", StringComparison.OrdinalIgnoreCase))
        {
            // Gutenberg metadata presence signals fiction
            if (metadata.ContainsKey("ebook_number"))
                signals.Add(new DetectionSignal { Name = "gutenberg_meta", Category = "metadata", Weight = 3.0, VotedType = Fiction, Reason = "Gutenberg eBook metadata present" });
        }
    }
    ...
    private static void CollectStructuralSignals(string earlyContent, List<DetectionSignal> signals)
    {
        ...
        if (ChapterMarkerRegex().IsMatch(earlyContent))
            signals.Add(new DetectionSignal { Name = "chapter_markers", Category = "structural", Weight = 5.0, VotedType = Fiction, Reason = "Chapter markers found" });

        // Act/Scene markers → play
        if (ActSceneRegex().IsMatch(earlyContent))
            signals.Add(new DetectionSignal { Name = "act_scene_markers", Category = "structural", Weight = 8.0, VotedType = Play, Reason = "Act/Scene markers found" });

        // Part/Book markers → anthology or long novel
        if (PartBookRegex().IsMatch(earlyContent))
            signals.Add(new DetectionSignal { Name = "part_markers", Category = "structural", Weight = 3.0, VotedType = Anthology, Reason = "Part/Book division markers found" });

        // Academic structure
        if (lower.Contains("abstract") && (lower.Contains("introduction") || lower.Contains("methodology")))
            signals.Add(new DetectionSignal { Name = "academic_structure", Category = "structural", Weight = 8.0, VotedType = Academic, Reason = "Abstract + Introduction/Methodology structure" });

        // Citation markers [1], [2]
        if (CitationRegex().IsMatch(earlyContent))
            signals.Add(new DetectionSignal { Name = "citations", Category = "structural", Weight = 4.0, VotedType = Academic, Reason = "Bracketed citation references found" });

        // Code blocks
        if (lower.Contains("```"))
            signals.Add(new DetectionSignal { Name = "code_blocks", Category = "structural", Weight = 5.0, VotedType = Technical, Reason = "Code blocks found" });

        // Page markers (PDF origin)
        if (lower.Contains("<!-- page:"))
            signals.Add(new DetectionSignal { Name = "page_markers", Category = "structural", Weight = 1.0, VotedType = Fiction, Reason = "PDF page markers present" });
    }
    ...
    private static void CollectContentSignals(string earlyContent, List<DetectionSignal> signals)
    {
        ...
        if (DialogueRegex().IsMatch(earlyContent))
            signals.Add(new DetectionSignal { Name = "dialogue", Category = "content", Weight = 5.0, VotedType = Fiction, Reason = "Dialogue patterns detected" });

        // Narrative pronouns with action verbs → fiction
        if (NarrativeRegex().IsMatch(lower))
            signals.Add(new DetectionSignal { Name = "narrative_voice", Category = "content", Weight = 3.0, VotedType = Fiction, Reason = "Narrative voice (he/she + action verbs)" });

        // Character honorifics → fiction
        if (HonorificsRegex().IsMatch(lower))
            signals.Add(new DetectionSignal { Name = "honorifics", Category = "content", Weight = 2.0, VotedType = Fiction, Reason = "Character honorifics (Mr./Mrs./Lord/etc.)" });

        // Stage directions → play
        if (StageDirectionRegex().IsMatch(earlyContent))
            signals.Add(new DetectionSignal { Name = "stage_directions", Category = "content", Weight = 6.0, VotedType = Play, Reason = "Stage directions found ([Enter...], [Exit...])" });

        // Speaker labels → play
        if (SpeakerLabelRegex().IsMatch(earlyContent))
            signals.Add(new DetectionSignal { Name = "speaker_labels", Category = "content", Weight = 5.0, VotedType = Play, Reason = "Speaker labels (CHARACTER_NAME. or CHARACTER_NAME:)" });

        // Academic keywords
        if (lower.Contains("methodology") || lower.Contains("literature review") || lower.Contains("hypothesis"))
            signals.Add(new DetectionSignal { Name = "academic_keywords", Category = "content", Weight = 3.0, VotedType = Academic, Reason = "Academic methodology keywords" });

        // Technical keywords
        if (lower.Contains("installation") || lower.Contains("getting started") || lower.Contains("configuration"))
            signals.Add(new DetectionSignal { Name = "technical_keywords", Category = "content", Weight = 3.0, VotedType = Technical, Reason = "Technical documentation keywords" });

        // ISBN / publisher → nonfiction
        if (lower.Contains("isbn") || lower.Contains("published by") || lower.Contains("copyright"))
            signals.Add(new DetectionSignal { Name = "publishing_meta", Category = "content", Weight = 3.0, VotedType = NonFiction, Reason = "Publishing metadata (ISBN, publisher)" });

        // Foreword/Preface → nonfiction
        if (lower.Contains("foreword") || lower.Contains("preface") || lower.Contains("acknowledgments"))
            signals.Add(new DetectionSignal { Name = "nonfiction_front", Category = "content", Weight = 3.0, VotedType = NonFiction, Reason = "Non-fiction front matter (foreword/preface)" });
        ...
        if (totalNonEmpty > 10 && (double)shortLines / totalNonEmpty > 0.6)
            signals.Add(new DetectionSignal { Name = "short_line_ratio", Category = "content", Weight = 4.0, VotedType = Collection, Reason = $"High ratio of short lines ({shortLines}/{totalNonEmpty}) — possible poetry" });
        ...
        if (lower.Contains("table of contents") || lower.Contains("contents"))
        {
            ...
            if (titleLikeCount > 10)
                signals.Add(new DetectionSignal { Name = "toc_many_entries", Category = "content", Weight = 4.0, VotedType = Collection, Reason = $"Table of Contents with {titleLikeCount} entries" });
        }

        // "Sonnet" / "Poem" / "Stanza" keywords
        if (lower.Contains("sonnet") || lower.Contains("stanza") || lower.Contains("verse"))
            signals.Add(new DetectionSignal { Name = "poetry_keywords", Category = "content", Weight = 5.0, VotedType = Collection, Reason = "Poetry-specific keywords (sonnet/stanza/verse)" });
    }

    [GeneratedRegex(@"^Chapter\s+(\d+|[IVXLC]+|One|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten)", RegexOptions.IgnoreCase | RegexOptions.Multiline)]
