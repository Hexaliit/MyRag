...
public sealed class CommunityDetector
{
    ...
    public async Task<HierarchicalCommunities> DetectAndSummarizeAsync(IProgress<ProgressInfo>? progress = null,
        CancellationToken ct = default)
    {
        ...
        for (var i = 0; i < communities.Count; i++)
        {
            ...
            var c = communities[i];
            progress?.Report(
                new ProgressInfo(i, communities.Count, $"Summarizing {c.Id} ({c.Entities.Count} entities)"));
    ...
    private (Dictionary<string, GraphNode> Nodes, double TotalWeight) BuildGraph(List<EntityResult> entities,
        List<RelationshipResult> rels)
    {
        ...
        foreach (var r in rels)
        {
            ...
            nodes[r.SourceEntityId].Neighbors[r.TargetEntityId] =
                nodes[r.SourceEntityId].Neighbors.GetValueOrDefault(r.TargetEntityId) + r.Weight;
            nodes[r.TargetEntityId].Neighbors[r.SourceEntityId] =
                nodes[r.TargetEntityId].Neighbors.GetValueOrDefault(r.SourceEntityId) + r.Weight;
        }
    ...
    private Dictionary<string, int> DetectCommunities(Dictionary<string, GraphNode> nodes, double totalWeight)
    {
        ...
        do
        {
            ...
            foreach (var nodeId in nodeList)
            {
                ...
                var neighborComms = node.Neighbors.Keys.Where(nodes.ContainsKey).Select(n => partition[n]).Distinct()
                    .ToList();
        ...
        var unique = partition.Values.Distinct().OrderBy(x => x).Select((v, i) => (v, i))
        ...
        string nodeId, int fromComm, int toComm, double m2)
    {
        ...
        var kiIn = node.Neighbors.Where(kv => nodes.ContainsKey(kv.Key) && partition[kv.Key] == toComm)
            .Sum(kv => kv.Value);
        var kiOut = node.Neighbors
            .Where(kv => nodes.ContainsKey(kv.Key) && partition[kv.Key] == fromComm && kv.Key != nodeId)
            .Sum(kv => kv.Value);

        var sigmaTot = partition.Where(kv => kv.Value == toComm).Sum(kv => nodes[kv.Key].Neighbors.Values.Sum());
        var sigmaFrom = partition.Where(kv => kv.Value == fromComm && kv.Key != nodeId)
            .Sum(kv => nodes[kv.Key].Neighbors.Values.Sum());
