...
public sealed class GraphRagDb : IDisposable
{
    ...
    {
        return ExecAsync("INSERT INTO chunks VALUES ($1,$2,$3,$4,$5,$6)",
    ...
    public async Task<List<ChunkResult>> GetAllChunksAsync()
    {
        ...
        return await ReadAsync(cmd,
            r => new ChunkResult(r.GetString(0), r.GetString(1), r.GetString(2), r.GetInt32(3)));
    }
    ...
    public async Task UpsertEntityAsync(string id, string name, string type, string? desc, string[] chunkIds)
    {
        ...
        if (chunkIds.Length > 0) await BatchUpsertMentionsAsync("entity_mentions", "entity_id", id, chunkIds);
    }
    ...
    public async Task<EntityResult?> GetEntityByNameAsync(string name)
    {
        ...
        var list = await ReadAsync(cmd,
            r => new EntityResult(r.GetString(0), r.GetString(1), r.GetString(2), r.IsDBNull(3) ? null : r.GetString(3),
                r.GetInt32(4)));
    ...
    public async Task UpsertRelationshipAsync(string id, string srcId, string tgtId, string relType, string? desc,
        string[] chunkIds)
    {
        // Upsert relationship
        await ExecAsync("""
    ...
    public async Task<List<RelationshipResult>> GetRelationshipsForEntityAsync(string entityId)
    {
        ...
        cmd.CommandText = """
                          ...
                          """;
        cmd.Parameters.Add(new DuckDBParameter { Value = entityId });
        return await ReadAsync(cmd, r => new RelationshipResult(r.GetString(0), r.GetString(1), r.GetString(2),
            r.GetString(3),
    ...
    {
        await ExecAsync(
            "INSERT INTO communities (id, level, summary) VALUES ($1, $2, $3) ON CONFLICT(id) DO UPDATE SET summary = EXCLUDED.summary",
            id, level, summary);

        foreach (var entityId in entityIds)
            await ExecAsync(
                "INSERT INTO community_members (community_id, entity_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", id,
                entityId);
    }

    public Task UpdateCommunitySummaryAsync(string id, string summary)
    {
        return ExecAsync("UPDATE communities SET summary = $2 WHERE id = $1", id, summary);
    }
    ...
    private static string Normalize(string s)
    {
        ...
        var result = Regex.Replace(s.Trim(), @"\s+", " ").ToLowerInvariant();
    ...
    private static async Task<List<T>> ReadAsync<T>(DuckDBCommand cmd, Func<IDataReader, T> map)
    {
        var list = new List<T>();
