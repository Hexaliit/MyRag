...
public static class LanguageConverter
{
    ...
    private static readonly Dictionary<string, string> LanguageNameMap = new()
    {
        { "es", "spanish" },
        { "fr", "french" },
        { "de", "german" },
    ...
    {
        return LanguageLocaleMap.TryGetValue(code.ToLower(), out var locale) ? locale : "en-GB";
    }

    public static string ConvertCodeToLanguage(this string code)
    {
        return LanguageMap.TryGetValue(code.ToLower(), out var languageName) ? languageName : "Unknown Language";
    }

    public static string ConvertCodeToLanguageName(this string code)
    {
        return LanguageNameMap.TryGetValue(code.ToLower(), out var languageName) ? languageName : "Unknown Language";
    }
}