    ...
    foreach ($gif in $testGifs)
    {
        ...
        try
        {
            ...
            if ($jsonOutput)
            {
                ...
                Write-Host "    Caption Score: $( $captionQuality.Score )/100 ($( $captionQuality.MatchedKeywords )/$( $captionQuality.TotalKeywords ) keywords)" -ForegroundColor $( if ($captionQuality.Score -ge 70)
                {
                    "Green"
                }
